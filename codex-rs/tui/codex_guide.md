...
- Description: when `[features].workflows = true`, regular `codex` starts a loopback websocket app-server at `ws://127.0.0.1:8765/` and connects the TUI to it. `/workflow <command>` launches a JavaScript workflow process with `CODEX_APP_SERVER_URL`, `CODEX_WORKFLOW_APP_SERVER_URL`, and `CODEX_WORKFLOW_APPROVALS=delegate` set, so workflow-started threads appear in `/agent` and replay progress, approvals, and final results through the same transcript path as TUI-started agents. Dynamic tool requests are displayed through normal item notifications, but the TUI does not answer `item/tool/call`; the workflow client that registered the dynamic tool must answer it. The launcher also sets legacy `CODEX_WORKFLOW_INTERACTIVE_REQUEST_BEHAVIOR=defer` for older SDK builds.
- Configuration: add `[features] workflows = true` to `config.toml`, restart `codex`, then run `/workflow node ./workflow.js`. External clients can also connect to `CODEX_APP_SERVER_URL=ws://127.0.0.1:8765/` while that TUI is running. If port 8765 is occupied, managed startup fails or connects to the existing Codex app-server on that port.
- Debug recipe: trace managed startup through `run_main`, `start_managed_workflow_app_server`, and `connect_remote_app_server_with_retries`; trace app-server event routing through `server_notification_thread_target`, `handle_app_server_event`, `enqueue_thread_notification`, and `attach_live_thread_for_selection`; inspect `ThreadEventStore.live_attached` when a `/agent` row exists but is not receiving live events. For dynamic tools, confirm the workflow client receives and answers `item/tool/call` while the TUI ignores it in `PendingAppServerRequests`.
- Source entrypoints: `codex-rs/tui/src/lib.rs`, `codex-rs/tui/src/app/workflows.rs`, `codex-rs/tui/src/chatwidget/slash_dispatch.rs`, `codex-rs/tui/src/app/app_server_events.rs`, `codex-rs/tui/src/app/session_lifecycle.rs`, `codex-rs/tui/src/app/thread_events.rs`, `codex-rs/tui/src/app/app_server_requests.rs`, `codex-rs/app-server/src/outgoing_message.rs`, `sdk/typescript/src/workflow.ts`.
- Token impact: observing external workflow threads does not add model-visible context to the active TUI thread. Switching to a workflow thread replays that thread's own transcript only.
