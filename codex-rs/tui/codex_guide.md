...
- Description: slash commands are parsed by the bottom-pane composer and dispatched by `ChatWidget` without involving the model unless the command explicitly submits a user turn or switches collaboration mode. Registered workflow aliases from `workflow.yaml.command` also appear here when workflows are enabled, and the shared workflow parser handles both `/<cmd>` and `codex <cmd>` while built-in slash commands keep precedence on collisions.
...
## App-Server Workflow Threads

- Description: when `[features].workflows = true`, regular `codex` starts a loopback websocket app-server at `ws://127.0.0.1:8765/` and connects the TUI to it. Bare `/workflow` enters Workflow collaboration mode for workflow design, tuning, exploration, validation, repair, and explanation; it is not a filesystem discovery mode. Workflows should emit `WorkflowContext.progress(message, data?)` while they run, and call `WorkflowContext.reportToUserMarkdown(markdown)` only when the next plain user turn should receive the workflow output. The TUI shows workflow progress in the live status row, renders reported markdown as a `Workflow Result` history cell, and carries that markdown forward as hidden context for the next user submission. Default scope is the current Codex workflow skills/config and the current repository. `/workflow done` exits to Default mode; `/workflow <command>` launches `codex workflow <command>` with `CODEX_APP_SERVER_URL`, `CODEX_WORKFLOW_APP_SERVER_URL`, `CODEX_WORKFLOW_RUN_ID`, `CODEX_WORKFLOW_ORIGIN_THREAD_ID` when available, and `CODEX_WORKFLOW_APPROVALS=delegate` set. Workflow-started threads appear in `/agent` and replay progress, approvals, and final results through the same transcript path as TUI-started agents. Dynamic tool requests are displayed through normal item notifications, but the TUI does not answer `item/tool/call`; the workflow client that registered the dynamic tool must answer it. Workflow tool visibility is registration-backed: `codex-rs/workflows/src/publication.rs` reads durable `workflow-tools` rows from artifactory, refreshes stale entries, and only falls back to filesystem bootstrap when no active registration exists yet.
- Configuration: add `[features].workflows = true` to `config.toml`, restart `codex`, then use `/workflow` as the specialist mode for workflow work, `/workflow list` to enumerate workflows, `/workflow show <id>` to inspect one, `/workflow where <id>` to locate a known workflow path, `/workflow develop <description>` to scaffold one, or `codex workflow <command>`. `codex workflow run <id> --key value` now builds a top-level JSON object for workflow input, while `--input '{...}'` or `--input @file.json` still passes raw JSON and any extra `--key value` flags override matching object keys. Workflow layout is strict: implementation code belongs in `src/`, tests belong in `src/tests/`, and persistent state or database files belong in `state/`. Every workflow should keep `README.md` and `DESIGN.md` current, track its `validation.coverage` contract in `workflow.yaml`, and annotate test files with `// workflow-covers: ...` markers so `/workflow validate <id>` can mechanically verify the coverage contract. Workflows must not depend on globally installed third-party packages; built-in platform modules are fine, but external packages must come from the workflow directory's local `package.json` and `node_modules`. Workflows should be as stable as possible and recover when correctness is preserved. Workflow implementation is staged: a persistent `workflow-architect` owns `DESIGN.md`, fresh `workflow-arch-reviewer` agents review each design round until `0 findings`, then a persistent `workflow-coder` implements the workflow while fresh `workflow-code-reviewer` agents review each coding round until `0 findings`. `DESIGN.md` may only be changed by the architect; coder-side design changes must be raised as `DESIGN.md request`s and routed back through the architect review loop before coding resumes. `/workflow validate <id>` checks docs, layout, coverage markers, local packages, and runs the workflow validation commands/tests before reporting readiness. Workflow roots are `$CODEX_HOME/workflows`, `.codex/workflows`, and `[workflows].search_paths`; `[workflows]` also supports `default_location`, `repair_mode`, `max_repair_cycles`, `dependency_update_policy`, `commit_policy`, and `validation_profile`. Each workflow directory is its own git repo with `workflow.yaml`, `README.md`, `DESIGN.md`, and TypeScript source. External clients can connect to `CODEX_APP_SERVER_URL=ws://127.0.0.1:8765/` while the TUI is running.
...
- Debug recipe: trace managed startup through `run_main`, `start_managed_workflow_app_server`, and `connect_remote_app_server_with_retries`; trace `/workflow` through `slash_dispatch.rs`, `codex-rs/tui/src/app/workflows.rs`, and `codex-rs/cli/src/workflow_cmd.rs`; trace the JavaScript workflow runtime bridge through `codex-rs/workflows/src/execute.rs` and `codex-rs/workflows/src/workflow_runtime.rs`; trace registration-backed discovery through `codex-rs/workflows/src/publication.rs`, `codex-rs/workflows/src/registry.rs`, and `codex-rs/artifactory/src/workflow_tool_registration.rs`; trace app-server APIs through `request_processors/workflow_processor.rs`. For workflow API discovery, compare `codex api --mcp-detail tools-and-auth-only`, SDK `ctx.api.read()`, SDK `ctx.workflows.registry.list()`, and app-server `apiCatalog/read` output. To inspect workflow tool registrations read-only, use `sqlite3 -readonly "$CODEX_HOME/artifactory_1.sqlite" "SELECT id, scope_key, source_key, state_dir, metadata_json, created_at_unix_sec, updated_at_unix_sec, last_hit_at_unix_sec FROM artifact_states WHERE namespace = 'workflow-tools' ORDER BY updated_at_unix_sec DESC;"` and then `sqlite3 -readonly "$CODEX_HOME/artifactory_1.sqlite" "SELECT path, kind, sha256 FROM artifact_sources WHERE state_id = ?1 ORDER BY path;"` for the chosen `state_id`; use `json_extract(metadata_json, '$.workflowId')`, `$.toolName`, `$.sourceHook`, `$.sourceDigest`, `$.publishedAtUnixSec`, `$.updatedAtUnixSec`, `$.refreshAfterUnixSec`, and `$.expiresAtUnixSec` to inspect the durable record itself.
- Source entrypoints: `codex-rs/workflows/src/execute.rs`, `codex-rs/workflows/src/workflow_runtime.rs`, `codex-rs/workflows/src/publication.rs`, `codex-rs/workflows/src/registry.rs`, `codex-rs/artifactory/src/workflow_tool_registration.rs`, `codex-rs/workflows/src/`, `codex-rs/collaboration-mode-templates/templates/workflow.md`, `codex-rs/tui/src/app/workflows.rs`, `codex-rs/tui/src/app/thread_routing.rs`, `codex-rs/tui/src/chatwidget/workflows.rs`, `codex-rs/tui/src/history_cell.rs`, `codex-rs/tui/src/bottom_pane/chat_composer.rs`, `codex-rs/tui/src/chatwidget/slash_dispatch.rs`, `codex-rs/core/src/context/workflow_instructions.rs`, `codex-rs/core/src/context/workflow_markdown_handoff.rs`, `codex-rs/app-server/src/request_processors/workflow_processor.rs`, `codex-rs/app-server-protocol/src/protocol/v2/workflow.rs`, `codex-rs/cli/src/workflow_cmd.rs`, `sdk/typescript/src/workflow.ts`.
