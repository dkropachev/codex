...
- Codex mode: bare `/codex` switches the current thread into Codex config planning mode with the same interaction model as `/plan`. `/codex <request>` switches to that mode and submits the request. The turn embeds the Plan-mode guide plus this guide, generated slash-command registry context, and current `codex --help` output as hidden developer instructions; the visible user message remains only the user-authored prompt. Planning turns run from a scratch workspace under the system temp directory with `/tmp` writable; the target workspace and Codex config directory are read-only during planning. The model must emit one complete `<proposed_plan>` for config changes, and the TUI opens the normal "Implement this plan?" prompt. Users can leave with `/codex off`, `/codex disable`, or `/codex cancel`.
- Codex config-edit mode: accepting the Codex plan starts the follow-up turn in an internal config-edit submode. That apply turn may modify only the Codex config directory and `/tmp`; the target workspace remains read-only. The apply turn should validate the change and reload config or state when possible, or describe any required restart and rollback path. It must not emit a new `<proposed_plan>`.
- Config history: config-edit apply turns currently rely on the model to inspect and validate the resulting `$CODEX_HOME` files; they do not create a dedicated config-history bundle. Prefer supported config APIs, app-server APIs, routed config tools, or maintained CLI commands before direct file edits, and include manual rollback notes in the final response when editing config.
- Token impact: local commands like `/status` and `/debug-config` add no model context by themselves; bare `/codex` adds Codex config planning developer instructions to subsequent turns, and commands such as `/codex <instruction>`, `/init`, `/compact`, `/review`, `/plan <prompt>`, and `/repo-ci <task>` can submit model-visible input.

## App-Server Workflow Threads

- Description: when `[features].workflows = true`, regular `codex` starts a loopback websocket app-server at `ws://127.0.0.1:8765/` and connects the TUI to it. Bare `/workflow` enters Workflow collaboration mode; `/workflow done` exits to Default mode; `/workflow <command>` launches `codex workflow <command>` with `CODEX_APP_SERVER_URL`, `CODEX_WORKFLOW_APP_SERVER_URL`, and `CODEX_WORKFLOW_APPROVALS=delegate` set. Workflow-started threads appear in `/agent` and replay progress, approvals, and final results through the same transcript path as TUI-started agents. Dynamic tool requests are displayed through normal item notifications, but the TUI does not answer `item/tool/call`; the workflow client that registered the dynamic tool must answer it.
- Configuration: add `[features] workflows = true` to `config.toml`, restart `codex`, then use `/workflow` for guarded workflow authoring, `/workflow list`, `/workflow develop <description>`, or `codex workflow <command>`. Workflow roots are `$CODEX_HOME/workflows`, `.codex/workflows`, and `[workflows].search_paths`; `[workflows]` also supports `default_location`, `repair_mode`, `max_repair_cycles`, `dependency_update_policy`, `commit_policy`, and `validation_profile`. Each workflow directory is its own git repo with `workflow.yaml`, `README.md`, and TypeScript source. External clients can connect to `CODEX_APP_SERVER_URL=ws://127.0.0.1:8765/` while the TUI is running.
...
- Debug recipe: trace managed startup through `run_main`, `start_managed_workflow_app_server`, and `connect_remote_app_server_with_retries`; trace `/workflow` through `slash_dispatch.rs` and `codex-rs/cli/src/workflow_cmd.rs`; trace registry discovery through `codex-rs/workflows/src/registry.rs`; trace app-server APIs through `request_processors/workflow_processor.rs`. For workflow API discovery, compare `codex api --mcp-detail tools-and-auth-only`, SDK `ctx.api.read()`, SDK `ctx.workflows.registry.list()`, and app-server `apiCatalog/read` output.
- Source entrypoints: `codex-rs/workflows/src/`, `codex-rs/collaboration-mode-templates/templates/workflow.md`, `codex-rs/tui/src/app/workflows.rs`, `codex-rs/tui/src/bottom_pane/chat_composer.rs`, `codex-rs/tui/src/chatwidget/slash_dispatch.rs`, `codex-rs/core/src/context/workflow_instructions.rs`, `codex-rs/app-server/src/request_processors/workflow_processor.rs`, `codex-rs/app-server-protocol/src/protocol/v2/workflow.rs`, `codex-rs/cli/src/workflow_cmd.rs`, `sdk/typescript/src/workflow.ts`.
- Token impact: observing external workflow threads does not add model-visible context to the active TUI thread. Selecting or typing a workflow mention adds that workflow's metadata and README as user-context for the submitted turn, so keep workflow docs concise.
