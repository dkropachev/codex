...
- Description: Codex injects a built-in system `PostToolUse` hook for `Bash|apply_patch`. After those mutating tool calls, the hidden `workflow-quality-hook` helper checks dirty workflows under the active `CODEX_HOME` against the current workflows config and blocks when a changed workflow fails validation. The hook returns reviewer-style `WF-*` findings through both the blocked tool result and hook additional context, so the same workflow development turn can continue by fixing them. This hook is managed by Codex, not by repo-local workflow files.
