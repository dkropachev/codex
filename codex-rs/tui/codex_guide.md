...
- Agent prompt/tool policy: app-server v2 `thread/start`, `thread/resume`, `thread/promptContext/read`, `thread/promptContext/update`, and `turn/start` accept or inspect experimental `promptContext` and `toolPolicy`. Workflow SDK `ctx.createAgent`/`ctx.resumeAgent` forward `systemInstructions`, developer/user-context block choices, and builtin/MCP/dynamic tool visibility; `agent.createAgent`/`agent.spawnAgent` inherit the same options, and `agent.run` can update developer/user-context blocks and tool visibility for the live thread while `systemInstructions` is start/resume/thread-update only. Workflow implementations that spawn agents should choose `PromptContextPreset.Workflow`, `PromptContextPreset.Minimal`, or `PromptContextPreset.Isolated`, omit unneeded blocks with `PromptBlockMode.Omit`, and use `toolPolicy` allow-lists to keep prompt and tool context small. `promptContext.strict` defaults to `true` and rejects policy combinations the server cannot honor exactly; use `strict: false` only for best-effort prompt shaping. `InstructionMode.Update` reads server-resolved instruction text through `thread/promptContext/read` before sending a concrete update. Debug prompt policy at `codex-rs/core/src/prompt_context.rs`, app-server mapping at `codex-rs/app-server/src/prompt_policy.rs`, and SDK conversion at `sdk/typescript/src/workflow.ts`.
