...
- Description: when `[features].workflows = true`, `/workflow` enters Workflow collaboration mode and `/workflow <command>` launches `codex workflow <command>`. Workflows declare `workflow.yaml.runtime.kind = rune|typescript` plus an `entrypoint`; omitted legacy manifests are TypeScript at `src/workflow.ts`, while new `/workflow develop <description>` scaffolds embedded Rune at `src/workflow.rn` unless `--runtime typescript` is passed. Rune workflows call `pub async fn run(ctx, input)`, optional `complete(ctx, input)`, and optional `to_tui_markdown(result)`; TypeScript workflows keep the resident Node host with a named default-export async function, optional `complete(...)`, and optional `WorkflowOutput.toTuiMarkdown(result)`. Both runtimes emit the same `__CODEX_WORKFLOW_EVENT__` status/markdown events, use `ctx.status(...)`, `ctx.progress(...)`, `ctx.reportToUserMarkdown(...)`, `ctx.runWorkflow(...)`, and cwd fields, and fall back to pretty JSON when no formatter exists. Rune also exposes `ctx.createAgent/startAgent/resumeAgent`, `AgentHandle`, `TurnHandle`, `TurnStream`, `ctx.defineTool`, `ctx.input.*`, app-server namespaces (`ctx.api`, `ctx.artifacts`, `ctx.workflows`, `ctx.mcp`, `ctx.tools`, `ctx.fs`, `ctx.process`), `ctx.appServer.configure(...)`, `ctx.appServer.setApprovals(...)`, `ctx.appServer.tryRequest(...)`, and raw `ctx.appServer.request(method, params)` access. Rune workflow state must stay app-server-backed; do not read Codex SQLite directly. Workflow authoring commands stage edits in `.workflow-staging/sessions/<session-id>` during live workflow sessions; `/workflow done` publishes staged changes and exits to Default mode.
- Configuration: enable `[features].workflows = true`, restart, then use `/workflow list`, `/workflow show <id>`, `/workflow where <id>`, `/workflow develop [--runtime rune|typescript] <description>`, `/workflow repair <id>`, or `codex workflow repair|fix <id>`. CLI/app-server calls can pass `stageSessionId`; direct execution accepts `--key value`, `--input '{...}'`, or `--input @file.json`. Runtime entrypoints must be relative paths inside the workflow directory. Rune contracts come from `workflow.yaml api.inputSchema/outputSchema/formatSchemas` with `api.callableName` or a lower-camel workflow id fallback; TypeScript contracts are extracted from `WorkflowInput`, `WorkflowOutput`, and optional `WorkflowFormats` exports. `/workflow validate <id>` checks runtime-aware required files, docs, layout, runtime state ignore/tracking, coverage markers, TypeScript local package imports when applicable, Rune source compilation, JSON schema syntax, output schema shape, contract smoke, and validation commands/tests. Rune workflows must define `validation.contractSmoke`; it can provide multiple `cases` with `input`, `expectOutput`, and `expectError`, validates manifest-declared format schemas through `to_tui_markdown(result)`, and checks `complete(ctx, input)` when autocomplete coverage is enabled. Rune validation commands default to `codex workflow test-rune`, which compiles exported test functions under `src/tests/**/*.test.rn` and `src/tests/**/*.spec.rn`; legacy `true` commands map to that built-in runner. Passing validation publishes the refreshed source-digested contract and regenerates generated workflow callers. Workflow roots are `$CODEX_HOME/workflows`, `.codex/workflows`, and `[workflows].search_paths`; `[workflows]` also supports `default_location`, `repair_mode`, `max_repair_cycles`, `dependency_update_policy`, `commit_policy`, and `validation_profile`.
