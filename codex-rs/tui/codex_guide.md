...
- Configuration: enable `[features].workflows = true`, restart, then use `/workflow list`, `/workflow show <id>`, `/workflow where <id>`, `/workflow develop [--runtime rune|typescript] <description>`, `/workflow repair <id>`, or `codex workflow repair|fix <id>`. CLI/app-server calls can pass `stageSessionId`; direct execution accepts `--key value`, `--input '{...}'`, or `--input @file.json`. Rune contracts come from `workflow.yaml api.inputSchema/outputSchema/formatSchemas` with `api.callableName` or a lower-camel workflow id fallback; TypeScript contracts are extracted from `WorkflowInput`, `WorkflowOutput`, and optional `WorkflowFormats` exports. `/workflow validate <id>` checks runtime-aware required files, docs, layout, runtime state ignore/tracking, coverage markers, TypeScript local package imports when applicable, Rune source compilation, output schema shape, contract smoke, and validation commands/tests. Rune `validation.contractSmoke` runs the embedded workflow with the configured `input` unless a shell `command` is supplied. Passing validation publishes the refreshed contract and regenerates generated workflow callers. Workflow roots are `$CODEX_HOME/workflows`, `.codex/workflows`, and `[workflows].search_paths`; `[workflows]` also supports `default_location`, `repair_mode`, `max_repair_cycles`, `dependency_update_policy`, `commit_policy`, and `validation_profile`.
