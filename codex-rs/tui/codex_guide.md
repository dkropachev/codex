...
- Description: slash commands are parsed by the bottom-pane composer and dispatched by `ChatWidget` without involving the model unless the command explicitly submits a user turn or switches collaboration mode. Registered workflow aliases from `workflow.yaml.command` or compiled-in native workflow metadata also appear here when workflows are enabled, and the popup can surface workflows by alias, id, title, or search terms so partial workflow names can still land on the right command. When the typed workflow alias is exact, the popup also shows dimmed workflow option hints extracted from the workflow input schema, filters those hints and live workflow suggestions from an optional `complete(ctx, request)` hook by the typed argument prefix, and treats cached option hints as display-only so `Tab` cannot commit placeholders like `<string>`. Live completion requests carry structured partial input, active field, prefix, and field/value mode; they are debounced, stale requests are cancelled, runtime errors surface in the popup, and exactly one live suggestion collapses to an inline dimmed suffix. Native workflows use their Rust completion hook when present and otherwise fall back to schema-derived completions without starting Bun. `Tab` accepts that preview text before submission while preserving multiline draft tails. `Enter` runs an unambiguous workflow search-term match without requiring an extra selection move. The shared workflow parser handles both `/<cmd>` and `codex <cmd>` while built-in slash commands keep precedence on collisions.
...
- Description: when `[features].workflows = true`, `/workflow` enters Workflow collaboration mode. `/workflow <command>` starts executable workflows through app-server v2 `workflowRun/start` and handles workflow management commands through the existing app-server workflow command APIs. App-server owns run ids, in-memory run state, cancellation, progress forwarding, markdown handoff, and completion/failure notifications on the active app-server transport. JavaScript execution still uses the Bun-backed workflow host behind app-server; runs are serialized through that host and each workflow import is cache-busted per execution. Compiled-in Rust workflows are first-class workflow summaries with `engine = rust`; they share discovery, aliases, `codex workflow run`, `workflowRun/start`, progress/status events, markdown handoff, input schemas, and output validation without a workflow directory or Bun process. Workflows are authored from `src/workflow.ts` as a default `defineWorkflow<Input, Output>({ id?, title?, callableName?, run, complete?, format? })` object with exported `WorkflowInput` and `WorkflowOutput` types; validation extracts that TypeScript contract, hook metadata, and formatter metadata as the source of truth. Native workflows implement `codex_native_workflow::NativeWorkflow` and provide compiled metadata, schema, defaults, progress/status events, and final markdown from Rust crates. `WorkflowContext.status({ workflowName, workflowStatus, threads? })` renders as `Workflow <workflowName>: <workflowStatus>`, adds `-> <threadName>: <threadStatus>` rows for any supplied thread list, and treats `WorkflowContext.progress(message, data?)` as a legacy single-status shorthand. `WorkflowContext.reportToUserMarkdown(markdown)` still becomes a `Workflow Result` history cell that is carried forward as hidden follow-up context for the originating thread. `WorkflowContext.artifacts.cache.ensure(...)` is the workflow-facing artifact API: declare a file/glob scope, let Codex hash it, and build into the provided output directory only when the scope changes or output is missing. Omitted output directories are managed under `$CODEX_HOME/.tmp/workflow-artifacts` and cleaned when app-server initializes after a restart. The workflow context points at the invoking workspace through `ctx.cwd`, `ctx.currentWorkingDirectory`, `ctx.repoRoot`, and `ctx.workingDirectory`; `process.cwd()` remains the workflow package directory. Workflow authoring commands such as `develop`, `describe`, `docs`, `edit`, and `fix` stage edits in a session-private `.workflow-staging/sessions/<session-id>` tree when they run from a live workflow session; validation runs against the staged copy, and `/workflow done` publishes staged changes back to the live workflow. CLI and app-server workflow commands still default to the one-shot path, but they can opt into the same staged behavior with an explicit session id (`codex workflow --stage-session-id <id> ...` or app-server `stageSessionId`), then publish or discard that staged session explicitly with `publish` or `discard`. When no formatter exists, the TUI falls back to pretty-printed JSON from the canonical workflow result. `/workflow done` exits to Default mode.
- Configuration: add `[features].workflows = true` to `config.toml`, restart `codex`, then use `/workflow` as the specialist mode for workflow work, `/workflow list` to enumerate workflows, `/workflow show <id>` to inspect one, `/workflow where <id>` to locate a known workflow path, `/workflow develop --location project --id <id> --command <command> [--title <title>] <description>` to scaffold a project workflow, `/workflow repair <id>` to run the validation-guided repair loop, or `codex workflow repair <id>` / `codex workflow fix <id>`. Direct workflow execution and registered aliases accept top-level object input via `--key value`; bare boolean flags map to `true`, `--input '{...}'` or `--input @file.json` passes a base JSON object, and explicit flags override keys from that object. Top-level TypeScript fields become kebab-case flags (`reviewId` becomes `--review-id`), string arrays accept either comma-separated values or JSON arrays, and nested objects require JSON. For app-server-owned runs, the workflow runner sets `CODEX_WORKFLOW_RUN_ID`, `CODEX_WORKFLOW_ORIGIN_THREAD_ID`, `CODEX_WORKFLOW_APPROVALS=delegate`, and `CODEX_WORKFLOW_APP_SERVER_URL` so JavaScript workflow code can report back over the same app-server socket. `CODEX_WORKFLOW_RUNTIME_MODE=process` remains a CLI/debug compatibility switch, but the app-server socket is the workflow control plane. Runtime startup uses `node_modules/.bin/bun` when a workflow supplies one; otherwise Codex uses its pinned managed Bun at `CODEX_HOME/workflows/.bin/bun`. Codex-launched Bun processes get isolated Bun install, cache, global package, and runtime transpiler cache paths under `CODEX_HOME/workflows/.env`; Node/tsx workflow execution is not a fallback. When workflows are enabled, Codex prefetches the pinned Bun package into `CODEX_HOME/workflows/.bin/`; runtime startup, validation, API extraction, and dependency repair also download it on demand when the binary is missing, so scaffolded `bun build` and `bun test` commands do not require a user-installed Bun binary. Workflow API contracts are TS-first: export `WorkflowInput`, `WorkflowOutput`, optional formatter types, and default `defineWorkflow` from `src/workflow.ts`, then let `/workflow validate <id>` extract and publish the contract after all checks pass. `workflow.yaml` still owns metadata, dependency, validation, repair, tool registration, and git policy, but validation rejects `workflow.yaml.api` schemas and `usage.options` so API drift cannot override the TypeScript contract. If a workflow sets `validation.contractSmoke`, live validation runs that command, parses stdout as JSON, and refuses to publish when the output violates the extracted contract. Discovery and command completion prefer that last validated contract over any live, unvalidated source edits. Workflow layout is strict: implementation code belongs in `src/`, tests belong in `src/tests/`, and persistent state or database files belong in `state/`; `.gitignore` must ignore `node_modules/`, `artifacts/`, and `state/*` while allowing `state/.gitkeep`, and runtime state files must not be tracked. Every workflow must carry a local `tsconfig.json`, non-empty required `README.md` and `DESIGN.md` sections, a private ESM `codex-workflow-*` package, and Bun-backed `build`, `test`, and `run` package scripts. Every object output schema must declare non-empty `properties` or an explicit `additionalProperties` policy. Every workflow should keep `README.md` and `DESIGN.md` current, keep `validation.coverage` aligned with the required `positive`, `load`, `autocomplete`, `negative`, and `recovery` markers, configure `validation.contractSmoke`, and annotate test files with `// workflow-covers: ...` markers so `/workflow validate <id>` can mechanically verify the coverage contract. Validation rejects unfinished scaffolds, including the default echo source, placeholder load/autocomplete/positive tests, raw develop flags in metadata/docs, legacy YAML API metadata, and `latest` dependency versions. Workflows must not depend on globally installed third-party packages; built-in platform modules are fine, but external packages must come from the workflow directory's local `package.json` and `node_modules`, unused runtime dependencies, Node/tsx/npm runtime scripts, Node engine pins, Node-only imports such as `node:sqlite`, npm/yarn/pnpm lockfiles, and `latest` package versions are rejected, and `workflow.yaml` `dependencies.runtime` / `dependencies.development` must match `package.json` `dependencies` / `devDependencies`. Workflows should be as stable as possible and recover when correctness is preserved. Workflow implementation is staged: a persistent `workflow-architect` owns `DESIGN.md`, fresh `workflow-arch-reviewer` agents review each design round until `0 findings`, then a persistent `workflow-coder` implements the workflow while fresh `workflow-code-reviewer` agents review each coding round until `0 findings`, followed by fresh `workflow-resilience-reviewer` agents reviewing runtime resilience until `0 findings`; if resilience findings require coder changes, rerun code review before rerunning resilience review so both gates are clean on the same implementation. `DESIGN.md` may only be changed by the architect; coder-side design changes must be raised as `DESIGN.md requests` and routed back through the architect review loop before coding resumes. `/workflow validate <id>` checks docs, layout, runtime state ignore/tracking, Bun-only package shape, dependency metadata, unused runtime dependencies, coverage markers, local packages, loadability, autocomplete readiness, output schema shape, contract smoke metadata/output, and runs the workflow validation commands/tests before reporting readiness; invalid validation returns a non-zero command exit code, and `/workflow run <id>` refuses invalid workflows before launch. `/workflow repair <id>` now emits progress while resolving, validating, applying deterministic fixes, refreshing local Bun dependencies when package or missing-module repairs need it, running AI fallback, committing, and completing; it reports the applied fixes, remaining findings, blocked findings, unsupported findings, validation command results, stop reason, and repair cycle counters in a structured `repair` result, and the command output message summarizes the applied fixes or stop reason with failure context. Workflow roots are `$CODEX_HOME/workflows`, `.codex/workflows`, and `[workflows].search_paths`; `[workflows]` also supports `default_location`, `repair_mode`, `max_repair_cycles`, `dependency_update_policy`, `commit_policy`, and `validation_profile`.
- Native workflow configuration: `[workflows]` also supports `engines` and `workflow_overrides`. Enable compiled-in Rust workflows with `[workflows.engines.rust] enabled = true`, disable all Rust workflows with `enabled = false`, or enable/disable one workflow with `[workflows.workflow_overrides."<id>"] enabled = true|false`. Configure `dev-cycle` defaults under `[workflows.workflow_overrides."dev-cycle".default_input]` using the workflow input field names, including `taskDescription`, `reviewTypes`, `reviewTypeDefinitions`, `experimentSampleRate`, `minEvidenceRuns`, `effortLoweringEnabled`, `groupingOptimizationEnabled`, `maxRejectedGroupingExperiments`, `maxReviewItemsPerGroup`, `baselineResampleRate`, `maxParallelReviewAgents`, `maxParallelWriters`, `maxParallelVerifiers`, `testMode`, `testCommands`, `defaultStageMode`, `integrationMode`, and nested `stages` keys such as `planning`, `implementation`, `tests`, and `integration`; explicit run input and CLI flags override per-workflow defaults, then native workflow defaults, then Rust-engine defaults. Built-in review type ids are `correctness`, `security`, `tests`, `architecture`, `api`, `maintainability`, `performance`, `ux`, `ui`, `accessibility`, and `docs`; custom review types require `shortName` and `description`, plus optional `prompt`, `excludePrompt`, and `enabled`. Review grouping starts at `separate:v1` for each exact model+effort, then the same target model may propose partitioned grouping challengers from same-model evidence; live status shows split selection, grouped review items, shadow-baseline phases, and a compact split outcome summary, while final markdown starts with a stable review-split summary and an explicit lost-evidence failure banner before the detailed active/primary/challenger splits, group composition, savings, proposal status, stop/suppression/promotion reasons, baseline resample results, model key, and repo T-shirt bucket. For run-time CLI input, prefer discoverable top-level flags such as `--review-types correctness,tests`, `--test-mode provided`, `--stage-tests off`, and `--stage-integration on` instead of hand-writing nested JSON. `dev-cycle` is a native Rust fix/review workflow: with an agent-capable host it runs planner, isolated writer worktree, reviewer, clean verifier, follow-up writer fix, test, and integrator stages; hosts that cannot spawn agents return a structured blocked result with selected review types and experiment state rather than the old preview. It is reachable as `/dev-cycle`, `codex dev-cycle`, `codex workflow run dev-cycle`, or the hidden diagnostic path `codex native-workflow run dev-cycle --input '{...}'`; list enabled native workflows with `codex native-workflow list --json`.
- Native dev-cycle SQLite recipe: state lives at `$CODEX_HOME/workflows/.native-workflows/dev-cycle/state/dev_cycle.sqlite3` and uses WAL mode. Inspect it read-only with `sqlite3 -readonly "$CODEX_HOME/workflows/.native-workflows/dev-cycle/state/dev_cycle.sqlite3"`; copy `dev_cycle.sqlite3`, `dev_cycle.sqlite3-wal`, and `dev_cycle.sqlite3-shm` together before offline inspection if Codex is running. Useful reads: `SELECT id,status,repo_identity,repo_family,repo_tshirt_bucket,work_size_units,datetime(started_at,'unixepoch') started,datetime(completed_at,'unixepoch') completed FROM runs ORDER BY started_at DESC LIMIT 20;`, `SELECT run_id,review_type_id,short_name,enabled FROM review_type_definitions ORDER BY run_id,review_type_id;`, `SELECT run_id,role,name,agent_id,model_key,status,datetime(created_at,'unixepoch') created FROM agent_attempts ORDER BY id DESC LIMIT 50;`, `SELECT id,run_id,review_type_id,title,severity,status,file_path,line FROM findings ORDER BY run_id,id;`, `SELECT finding_id,model_key,accepted,reason,work_size_units,datetime(created_at,'unixepoch') created FROM verifier_decisions ORDER BY id DESC LIMIT 50;`, `SELECT review_type_id,title,reason,status,datetime(created_at,'unixepoch') created FROM disregarded_findings ORDER BY id DESC LIMIT 50;`, `SELECT repo_family,review_type_id,model_key,runs,work_size_units,accepted,rejected,score FROM model_effort_scores ORDER BY review_type_id,model_key;`, `SELECT run_id,review_type_id,model_key,split_strategy,decision,reason FROM experiment_decisions ORDER BY id DESC LIMIT 50;`, `SELECT run_id,model_key,repo_tshirt_bucket,item_set_key,strategy_id,status,rejection_code,rejection_reason,datetime(created_at,'unixepoch') created FROM split_strategy_proposals ORDER BY id DESC LIMIT 50;`, `SELECT run_id,model_key,repo_tshirt_bucket,item_set_key,strategy_id,status,reviewer_count_savings,lost_evidence_count FROM split_strategy_attempts ORDER BY id DESC LIMIT 50;`, `SELECT model_key,repo_tshirt_bucket,item_set_key,strategy_id,runs,accepted,rejected,lost_evidence_count,reviewer_count_savings,score FROM split_scores ORDER BY updated_at DESC LIMIT 50;`, `SELECT model_key,repo_tshirt_bucket,item_set_key,strategy_id,item_neighborhood_key,reason FROM split_suppressions ORDER BY id DESC LIMIT 50;`, and `SELECT run_id,model_key,strategy_id,review_type_id,title,reason FROM split_lost_evidence ORDER BY id DESC LIMIT 50;`.
...
- Agent prompt/tool policy: app-server v2 `thread/start`, `thread/resume`, `thread/fork`, `thread/promptContext/read`, `thread/promptContext/update`, `turn/start`, and `command/exec` accept or inspect experimental `promptContext` and `toolPolicy` where applicable. Workflow SDK `ctx.createAgent`/`ctx.resumeAgent` forward `systemInstructions`, developer/user-context block choices, builtin/MCP/dynamic tool visibility, and invocation policy; `agent.createAgent`/`agent.spawnAgent` inherit the same options, and `agent.run` can update developer/user-context blocks and tool visibility for the live thread while `systemInstructions` is start/resume/thread-update only. Workflow implementations that spawn agents should choose `PromptContextPreset.Workflow`, `PromptContextPreset.Minimal`, or `PromptContextPreset.Isolated`, omit unneeded blocks with `PromptBlockMode.Omit`, and use `toolPolicy` allow-lists to keep prompt and tool context small. `toolPolicy.invocation` is evaluated before tool handlers run, applies to all tool payload kinds, and can match the raw input or a JSON subfield with `jsonPath`; omitted `jsonPath` defaults to `""`, meaning raw input. `invocation.mode: "default"` keeps built-in deny rules such as the broad `find /tmp / -name AGENTS.md` guard, `mode: "allowOnly"` requires an allow rule match, and `mode: "unrestricted"` is the explicit opt-out for agents or standalone commands that must allow anything. `promptContext.strict` defaults to `true` and rejects policy combinations the server cannot honor exactly; use `strict: false` only for best-effort prompt shaping. `InstructionMode.Update` reads server-resolved instruction text through `thread/promptContext/read` before sending a concrete update. Debug prompt and invocation policy at `codex-rs/core/src/prompt_context.rs`, tool dispatch enforcement at `codex-rs/core/src/tools/registry.rs`, app-server mapping at `codex-rs/app-server/src/prompt_policy.rs`, standalone command enforcement at `codex-rs/app-server/src/tool_invocation_policy.rs`, and SDK conversion at `sdk/typescript/src/workflow.ts`.
...
- Resume visibility: workflow socket `thread/start` and `thread/fork` sessions are persisted as workflow subagent sessions for recovery and analysis, but default `codex resume`, `codex resume --last`, and `/resume` only surface interactive sessions. Use `codex resume --include-non-interactive` to inspect workflow/subagent entries in the picker, `codex resume --include-non-interactive --last` to resume the newest non-interactive session, or a direct thread id when the id is already known. `codex exec resume` follows the same default and has the same `--include-non-interactive` opt-in.
...
- Token impact: prompt byte estimates and effective context windows directly affect candidate eligibility. The router also excludes a candidate from production and shadow for a `task_key` when the maximum observed `production` or `shadow` `model_router_ledger.total_tokens` for that route exceeds the candidate's usable context window; judge and router-overhead rows are excluded. Production ledger rows are written from routed chat and internal model responses; tune replay, judge, and shadow/monitoring evaluation rows are router overhead. Net savings is production counterfactual cost minus actual production cost minus model-router overhead.
...
- Model-router route context recipe:
