...
- Description: when `[features].workflows = true`, regular `codex` starts a loopback websocket app-server at `ws://127.0.0.1:8765/` and connects the TUI to it. Bare `/workflow` enters Workflow collaboration mode; `/workflow done` exits to Default mode; `/workflow <command>` launches `codex workflow <command>` with `CODEX_APP_SERVER_URL`, `CODEX_WORKFLOW_APP_SERVER_URL`, and `CODEX_WORKFLOW_APPROVALS=delegate` set. Workflow-started threads appear in `/agent` and replay progress, approvals, and final results through the same transcript path as TUI-started agents. Dynamic tool requests are displayed through normal item notifications, but the TUI does not answer `item/tool/call`; the workflow client that registered the dynamic tool must answer it. Workflow tool visibility is now registration-backed: `codex-rs/workflows/src/publication.rs` reads durable `workflow-tools` rows from artifactory, refreshes stale entries, and only falls back to filesystem bootstrap when no active registration exists yet.
