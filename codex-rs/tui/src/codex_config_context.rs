...
fn codex_config_sandbox_policy() -> SandboxPolicy {
...
fn build_codex_config_edit_context(target_cwd: &Path, workspace: &Path) -> String {
...
fn build_codex_ai_resolve_context(target_cwd: &Path, workspace: &Path) -> String {
    let mut sections = vec![
        PLAN_MODE_GUIDE.to_string(),
        format!(
