...
async fn workflow_output_records_assistant_message_for_next_context() {
    ...
    let response_item_id = raw_items
        .first()
        .and_then(ResponseItem::id)
        .cloned()
        .expect("workflow output should have a response item ID");
    expected_item.set_id(Some(response_item_id));
    assert_eq!(raw_items, std::slice::from_ref(&expected_item));
