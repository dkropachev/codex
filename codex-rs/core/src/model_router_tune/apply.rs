...
pub(super) async fn current_metric_state(
    ...
    if let Some(model_router) = config.model_router.as_ref() {
        for candidate in &model_router.candidates {
            ...
            current.insert(
                identity_key,
                metric_snapshots(Some(candidate), overlay.as_ref()),
            );
        }
    }
    for identity_key in candidate_identity_keys {
...
fn metric_snapshots(
    candidate: Option<&ModelRouterCandidateToml>,
    overlay: Option<&ModelRouterMetricOverlay>,
) -> Vec<ModelRouterMetricSnapshot> {
    vec![
        metric_snapshot(
            ModelRouterMetricName::IntelligenceScore,
            candidate
                .and_then(|candidate| candidate.intelligence_score)
        ...
        metric_snapshot(
            ...
            candidate
                .and_then(|candidate| candidate.success_rate)
                .map(ModelRouterMetricValue::Score),
            overlay
        ...
        metric_snapshot(
            ...
            candidate
                .and_then(|candidate| candidate.median_latency_ms)
