...
pub(crate) struct ModelRouterPromptEstimate {
    pub(crate) total_bytes: usize,
    pub(crate) cacheable_bytes: usize,
}

impl ModelRouterPromptEstimate {
    pub(crate) fn uncached(total_bytes: usize) -> Self {
        Self {
            total_bytes,
            cacheable_bytes: 0,
        }
    ...
    pub(crate) fn new(total_bytes: usize, cacheable_bytes: usize) -> Self {
        Self {
            total_bytes,
            cacheable_bytes: cacheable_bytes.min(total_bytes),
        }
    }
}
...
pub(crate) fn apply_model_router_with_exclusions(
    ...
    apply_model_router_with_prompt_estimate_and_exclusions(
        config,
        source,
        ModelRouterPromptEstimate::uncached(prompt_bytes),
        available_models,
        exclusions,
    )
}
...
#[cfg(test)]
pub(crate) fn apply_model_router_with_prompt_estimate_and_exclusions(
    config: &mut Config,
    source: ModelRouterSource,
...
pub(crate) async fn apply_model_router_with_state_and_exclusions(
    ...
    apply_model_router_with_state_prompt_estimate_and_exclusions(
        config,
        source,
        ModelRouterPromptEstimate::uncached(prompt_bytes),
        available_models,
...
pub(crate) async fn apply_model_router_with_state_prompt_estimate_and_exclusions(
    config: &mut Config,
    source: ModelRouterSource,
    prompt_estimate: ModelRouterPromptEstimate,
    available_models: &[AvailableRouterModel],
...
pub(crate) async fn record_model_router_request_usage(
    ...
    let counterfactual_usage = counterfactual_token_usage(token_usage, accounting);
...
async fn apply_lifecycle_transitions(
    ...
    if model_router.lifecycle.is_none() {
        return promotions;
    }
...
impl ModelRouterDiscoveryCache {
    ...
    async fn models_for_provider(
        ...
        let models = match list_provider_models_uncached(provider.clone()).await {
            Ok(model_infos) if !model_infos.is_empty() => model_infos
                .iter()
            ...
            Err(err) => {
                tracing::debug!(
                    provider_id,
                    error = %err,
                    "failed to discover custom provider models for model router"
                );
                Vec::new()
            }
        };
...
fn build_candidate_set(
    ...
    let task_class = RouterTaskClass::infer(task_key, prompt_estimate.total_bytes);
    routes.push(CandidateRoute {
        ...
        is_incumbent: true,
        metrics: incumbent_metrics(config, task_class, prompt_estimate, &candidates),
    });
    routes.extend(candidates.iter().map(|candidate| {
        ...
        let cache_compatible =
            route_preserves_prompt_cache(config, &model_provider, model.as_deref());
        CandidateRoute {
...
fn incumbent_metrics(
    ...
    CandidateMetrics {
        estimated_cost_usd_micros: incumbent_token_price(config, candidates).map(|price| {
            estimate_token_cost(
                &estimate_task_usage_for_route(
                    prompt_estimate,
                    task_class,
...
fn candidate_metrics(
    ...
    let explicit_estimated_cost_usd_micros = token_price_from_candidate(candidate).map(|price| {
        estimate_token_cost(
            &estimate_task_usage_for_route(prompt_estimate, task_class, cache_compatible),
            &price,
            /*confidence*/ 1.0,
        )
        .usd_micros
...
fn route_preserves_prompt_cache(
    config: &Config,
    model_provider_id: &str,
    model: Option<&str>,
) -> bool {
    model_provider_id == config.model_provider_id && model == config.model.as_deref()
}

fn estimate_task_usage_for_route(
    prompt_estimate: ModelRouterPromptEstimate,
    task_class: RouterTaskClass,
    cache_compatible: bool,
) -> TokenUsage {
    let mut usage = estimate_task_usage(prompt_estimate.total_bytes, task_class);
    if cache_compatible {
        usage.cached_input_tokens =
            estimated_cacheable_input_tokens(prompt_estimate).min(usage.input_tokens.max(0));
    }
    usage
}

fn estimated_cacheable_input_tokens(prompt_estimate: ModelRouterPromptEstimate) -> i64 {
    i64::try_from(prompt_estimate.cacheable_bytes.div_ceil(4)).unwrap_or(i64::MAX)
}
...
fn build_model_router_accounting(
    ...
    let counterfactual_cached_input_tokens = if selected_index == 0
        || route_preserves_prompt_cache(
            config,
            &selected_route.model_provider,
            selected_route.model.as_deref(),
        ) {
        0
    } else {
        estimated_cacheable_input_tokens(prompt_estimate)
    };
    ModelRouterAccounting {
        task_key: task_key.to_string(),
        ...
        counterfactual_cached_input_tokens,
    }
}
...
fn counterfactual_token_usage(
    ...
    usage.cached_input_tokens = usage
        .cached_input_tokens
        .max(accounting.counterfactual_cached_input_tokens)
...
mod tests {
    ...
    async fn available_router_models_discovers_custom_registered_provider() {
        ...
        let candidate_set = build_candidate_set(
            ...
            ModelRouterPromptEstimate::uncached(/*total_bytes*/ 80),
            &available_models,
            &[],
        )
    ...
    async fn custom_provider_discovery_failure_falls_back_without_error() {
        let server = MockServer::start().await;
        mount_models_response(&server, /*status*/ 500, Vec::new()).await;
        let mut config = config::test_config().await;
        config.model = Some("gpt-5.4".to_string());
        config.model_router = Some(ModelRouterToml {
            enabled: true,
    ...
    #[tokio::test]
    async fn cacheable_prompt_cost_keeps_incumbent_when_model_switch_loses_cache() {
        let mut config = config::test_config().await;
        config.model = Some("gpt-5.4".to_string());
        config.model_router = Some(ModelRouterToml {
            enabled: true,
            candidates: vec![
                ...
                ModelRouterCandidateToml {
                    id: Some("cheap-uncached".to_string()),
                    model: Some("gpt-5.2".to_string()),
                    intelligence_score: Some(0.94),
                    success_rate: Some(0.90),
                    median_latency_ms: Some(30_000),
        ...
        apply_model_router_with_prompt_estimate_and_exclusions(
            &mut uncached,
            ModelRouterSource::Chat(ModeKind::Default),
            ModelRouterPromptEstimate::uncached(/*total_bytes*/ 400_000),
            &[],
            &[],
        )
        .expect("router should apply");
        assert_eq!(uncached.model.as_deref(), Some("gpt-5.2"));

        apply_model_router_with_prompt_estimate_and_exclusions(
            &mut config,
            ModelRouterSource::Chat(ModeKind::Default),
            ModelRouterPromptEstimate::new(
                /*total_bytes*/ 400_000, /*cacheable_bytes*/ 360_000,
            ),
            &[],
            &[],
        )
    ...
    #[tokio::test]
    async fn routed_request_usage_counterfactual_keeps_incumbent_cached_input() {
        ...
        config.model_router = Some(ModelRouterToml {
            ...
            candidates: vec![
                ...
                ModelRouterCandidateToml {
                    id: Some("forced-switch".to_string()),
                    model: Some("gpt-5.2".to_string()),
                    input_price_per_million: Some(2.0),
                    output_price_per_million: Some(1.0),
                    ..Default::default()
            ...
            models: Some(ModelRouterModelsToml {
                rules: vec![ModelRouterModelRuleToml {
                    id: Some("force-switch".to_string()),
                    rule_type: ModelRouterModelRuleTypeToml::Require,
        ...
        assert_eq!(
            summary.totals.savings,
            RouterSavings {
                actual_production_cost_usd_micros: 200_000,
                router_overhead_cost_usd_micros: 0,
                counterfactual_cost_usd_micros: 109_000,
                gross_savings_usd_micros: -91_000,
                net_savings_usd_micros: -91_000,
            }
        );
    }
    ...
    async fn routed_request_usage_records_one_row_per_terminal_outcome() {
        ...
        let accounting = ModelRouterAccounting {
            ...
            counterfactual_cached_input_tokens: 0,
        };
