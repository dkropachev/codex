...
impl ResolvedEnvironment {
    fn into_turn_environment(
        self,
        mut selection: TurnEnvironmentSelection,
        config_origin: EnvironmentConfigOrigin,
    ) -> Option<TurnEnvironment> {
        if matches!(selection.config, EnvironmentConfigState::Pending) {
            selection.config = EnvironmentConfigState::Ready(self.installed_config?);
        }
        let mut environment =
            TurnEnvironment::new(selection, config_origin, self.environment, self.shell);
        environment.shell_snapshot = self.shell_snapshot;
        environment.shell_snapshot_v2_supported = self.shell_snapshot_v2_supported;
        Some(environment)
    }
}
    ...
    pub(crate) async fn resolve_primary(
        ...
        match self.environments.first() {
            ...
            Some(TurnEnvironmentState::Starting(environment)) => {
                ...
                Ok(resolved.into_turn_environment(
...
"#,
    ...
    async fn resolving_primary_waits_for_first_selection_before_ready_secondary() {
        ...
        let environment_config = test_environment_config();
        let environments = Arc::new(ThreadEnvironments::new(
            manager,
            crate::shell::default_user_shell(),
        ...
        environments.update_selections(&[remote.clone(), local], &environment_config);

        timeout(Duration::from_secs(5), async {
            loop {
                let snapshot = environments.snapshot().await;
                if snapshot.primary().is_some_and(|environment| {
                    environment.selection.environment_id == LOCAL_ENVIRONMENT_ID
                }) {
                    break;
                }
                tokio::task::yield_now().await;
        ...
        assert_eq!(primary.config(), &environment_config);
        server.await.expect("server task");
    }
