...
fn handoff_preserves_order_and_bounds_every_fragment() {
    ...
    let text = items
        ...
        .collect::<Vec<_>>();
    assert!(text.iter().all(|text| text.len() <= MAX_FRAGMENT_BYTES));
    assert!(text.iter().map(|text| text.len()).sum::<usize>() <= MAX_LOGICAL_BYTES + 1024);
...
fn handoff_includes_every_pending_report_within_the_logical_cap() {
        ...
        .map(|index| {
            ...
            oversized.output.findings = (0..64)
                .map(|finding_index| ReviewFinding {
                    title: format!("finding-{finding_index}-{}", "x".repeat(1_000)),
                    body: String::new(),
                    confidence_score: 1.0,
                    priority: 1,
                    code_location: ReviewCodeLocation {
    ...
    let total = handoff
        ...
        .sum::<usize>();
    assert!(total <= MAX_LOGICAL_BYTES + 64 * (OPEN_MARKER.len() + CLOSE_MARKER.len()));
}
