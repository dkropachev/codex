...
const MAX_MULTI_AGENT_ROLE_BODY_TOKENS: usize = 9_800;
...
impl MultiAgentRoleInstructions {
    pub(crate) fn unmarked(text: impl Into<String>) -> Self {
        Self {
            text: bounded_role_text(text.into()),
            marked: false,
        }
    }
...
fn bounded_role_text(text: String) -> String {
    truncate_text(
        &text,
        TruncationPolicy::Tokens(MAX_MULTI_AGENT_ROLE_BODY_TOKENS),
    )
}
