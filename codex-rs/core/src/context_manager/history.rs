...
const OUTPUT_OMISSION_NOTICE_HEADROOM_TOKENS: i64 = 50;
            ...
            ResponseItem::CustomToolCall {
                ...
                if let Ok(args) = codex_code_mode::parse_exec_source(input) {
                    let max_output_tokens = args
                        ...
                        .min(MAX_MODEL_CONTEXT_ITEM_TOKENS);
...
fn bound_model_context_output_item(mut item: ResponseItem) -> ResponseItem {
    ...
    match body {
        FunctionCallOutputBody::Text(text) => {
            ...
            let overhead_tokens = estimate_item_token_count(&item);
            let mut text_budget = i64::try_from(MAX_MODEL_CONTEXT_ITEM_TOKENS)
                .unwrap_or(i64::MAX)
                .saturating_sub(overhead_tokens)
                .saturating_sub(16);
            for _ in 0..4 {
                let bounded = truncate_text(
                    &original,
                    TruncationPolicy::Tokens(usize::try_from(text_budget).unwrap_or_default()),
                );
                ...
                let limit = i64::try_from(MAX_MODEL_CONTEXT_ITEM_TOKENS).unwrap_or(i64::MAX);
                if estimate <= limit {
                    return item;
                }
                text_budget = text_budget.saturating_sub(estimate.saturating_sub(limit) + 16);
            }
            if let ResponseItem::FunctionCallOutput { output, .. }
            | ResponseItem::CustomToolCallOutput { output, .. } = &mut item
            {
                output.body = FunctionCallOutputBody::Text(String::new());
            }
        }
        FunctionCallOutputBody::ContentItems(items) => {
            ...
            let retention_limit = i64::try_from(MAX_MODEL_CONTEXT_ITEM_TOKENS)
                .unwrap_or(i64::MAX)
                .saturating_sub(OUTPUT_OMISSION_NOTICE_HEADROOM_TOKENS);
            for _ in 0..MAX_OUTPUT_CONTENT_ITEMS_TO_SCAN {
                let Some(candidate) = candidates.next() else {
                    break;
                };
                output_content_items_mut(&mut item).push(candidate);
                if estimate_item_token_count(&item) > retention_limit {
                    output_content_items_mut(&mut item).pop();
                    omitted = omitted.saturating_add(1);
                }
            }
            omitted = omitted.saturating_add(candidates.count());
            if omitted > 0 {
                output_content_items_mut(&mut item).push(
                    FunctionCallOutputContentItem::InputText {
                        text: format!("[omitted {omitted} content items to fit context limit]"),
                    },
                );
                if estimate_item_token_count(&item)
                    > i64::try_from(MAX_MODEL_CONTEXT_ITEM_TOKENS).unwrap_or(i64::MAX)
    ...
    debug_assert!(
        estimate_item_token_count(&item)
            <= i64::try_from(MAX_MODEL_CONTEXT_ITEM_TOKENS).unwrap_or(i64::MAX),
        "function output metadata alone exceeded the context-item limit"
    );
    item
}

fn bound_model_context_call_id(item: &mut ResponseItem) {
    ...
    if call_id.len() <= MAX_MODEL_CONTEXT_CALL_ID_BYTES {
        return;
    }
    let digest = sha1_digest(call_id.as_bytes());
    *call_id = format!("bounded_{}", BASE64_STANDARD.encode(digest));
}

fn output_content_items_mut(item: &mut ResponseItem) -> &mut Vec<FunctionCallOutputContentItem> {
    let output = match item {
        ResponseItem::FunctionCallOutput { output, .. }
        | ResponseItem::CustomToolCallOutput { output, .. } => output,
        _ => unreachable!("only function output items are bounded"),
    };
    let FunctionCallOutputBody::ContentItems(items) = &mut output.body else {
        unreachable!("output body was initialized with content items")
    };
    items
}
