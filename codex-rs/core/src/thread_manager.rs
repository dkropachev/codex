...
impl ThreadManager {
    ...
    pub(crate) fn with_models_provider_and_home_for_tests(
        ...
        Self {
            state: Arc::new(ThreadManagerState {
                threads: Arc::new(RwLock::new(HashMap::new())),
                thread_created_tx,
                models_manager: create_model_provider("test", provider, Some(auth_manager.clone()))
                    .models_manager(
                        codex_home,
    ...
    where
        S: Into<ForkSnapshot>,
    {
        self.fork_thread_from_history_with_source(
            snapshot,
            config,
            history,
            self.state.session_source.clone(),
    ...
    /// Fork an existing thread from already-loaded store history using an explicit source.
    pub async fn fork_thread_from_history_with_source<S>(
        &self,
        snapshot: S,
        config: Config,
        history: InitialHistory,
