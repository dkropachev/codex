...
pub async fn tune_model_router(
    ...
    let candidates = tune_candidates(config, runtime.as_ref()).await?;
...
pub async fn refresh_model_router_report_deltas(
    ...
    let report_candidate_identity_keys = report
        .candidates
        .iter()
        .map(|candidate| candidate.identity_key.clone())
        .collect::<Vec<_>>();
    let current =
        apply::current_metric_state(state_db, config, &report_candidate_identity_keys).await?;
        ...
        let evaluation = if let Some(runtime) = runtime {
            ...
            .await
            {
                Ok(evaluation) => {
                    ...
                    if let Err(err) = persist_shadow_evaluations(
                        state_db,
                        &lifecycle_promotions,
                        &evaluation.shadow_evaluations,
                    )
                    .await
                    {
                        tracing::debug!(candidate_identity = identity_key, error = %err, "failed to persist model router shadow evaluations");
                    }
                    Some(Ok(evaluation))
                }
                Err(err) => {
...
async fn persist_shadow_evaluations(
    state_db: &StateRuntime,
    promotions: &[ModelRouterLifecyclePromotionRecord],
    evaluations: &[eval::CandidateShadowEvaluation],
) -> anyhow::Result<()> {
    let created_at_ms = Utc::now().timestamp_millis();
    for evaluation in evaluations {
        let phase = shadow_evaluation_phase(promotions, evaluation);
        state_db
            .record_model_router_shadow_evaluation(ModelRouterShadowEvaluationRecord {
                id: None,
                created_at_ms,
                task_key: evaluation.task_key.clone(),
                phase: phase.to_string(),
                candidate_identity: evaluation.candidate_identity.clone(),
                base_candidate_identity: evaluation.base_candidate_identity.clone(),
                success: evaluation.success,
                score: Some(evaluation.score),
                confidence: evaluation.confidence,
                cost_usd_micros: evaluation.cost_usd_micros,
...
fn shadow_evaluation_phase(
    promotions: &[ModelRouterLifecyclePromotionRecord],
    evaluation: &eval::CandidateShadowEvaluation,
) -> &'static str {
    if promotions.iter().any(|promotion| {
        promotion.task_key == evaluation.task_key
            && promotion.candidate_identity == evaluation.candidate_identity
...
mod tests {
    ...
    #[tokio::test]
    async fn tune_candidates_include_auto_discovered_models() {
        ...
        let models_manager = Arc::new(StaticModelsManager::new(
            /*auth_manager*/ None,
            ModelsResponse {
                models: vec![model_info_from_slug("gpt-5.3-codex-spark")],
            },
            Default::default(),
        ));
        let runtime = ModelRouterTuneRuntime::new(
            AuthManager::shared_from_config(&config, /*enable_codex_api_key_env*/ false),
            models_manager,
        );

        let candidates = tune_candidates(&config, Some(&runtime))
            .await
            .expect("tune candidates");
