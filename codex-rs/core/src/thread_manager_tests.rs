...
fn test_thread_manager(config: &Config, auth_manager: Arc<AuthManager>) -> ThreadManager {
    ThreadManager::new(
        config,
        auth_manager,
        SessionSource::Exec,
...
async fn resume_and_fork_do_not_restore_thread_environments_from_rollout() {
    ...
    let manager = test_thread_manager(&config, auth_manager.clone());
    let selected_cwd =
...
async fn new_uses_active_provider_for_model_refresh() {
    ...
    let auth_manager =
        AuthManager::from_auth_for_testing(CodexAuth::create_dummy_chatgpt_auth_for_testing());
    let manager = test_thread_manager(&config, auth_manager);
