...
"#,
    ...
    let project_layers: Vec<_> = layers
        ...
        .collect();
    assert!(
        project_layers.iter().all(|layer| {
            layer
                .config_folder()
                .is_none_or(|folder| folder.as_path() != codex_home.as_path())
        }),
        "expected CODEX_HOME to be ignored as a project layer"
    );
    assert_eq!(
    ...
    let project_layers_untrusted: Vec<_> = layers_untrusted
        ...
        .filter(|layer| {
            matches!(layer.name, ConfigLayerSource::Project { .. })
                && layer
                    .config_folder()
                    .is_some_and(|folder| folder.as_path().starts_with(tmp.path()))
        })
        .collect();
