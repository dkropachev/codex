...
async fn collect_tools(availability: CommandToolAvailability) -> Result<Vec<String>> {
    ...
    let mut builder = match availability {
        ...
        CommandToolAvailability::OutputCompactionEnabled => test_codex().with_config(|config| {
            config
                .features
...
async fn unified_exec_spec_toggle_end_to_end() -> Result<()> {
    ...
    for availability in [
        ...
        for command_tool in ["exec_command", "write_stdin", "read_exec_output"] {
            assert!(
    ...
    for availability in [
        ...
        assert!(
            !tools.iter().any(|name| name == "read_exec_output"),
            "tools list should not include read_exec_output for {availability:?}: {tools:?}"
        );
    }

    let tools = collect_tools(CommandToolAvailability::OutputCompactionEnabled).await?;
    for command_tool in ["exec_command", "write_stdin", "read_exec_output"] {
        assert!(
            tools.iter().any(|name| name == command_tool),
