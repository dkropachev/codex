...
fn assert_context_output_bounded(output: &str, expected_prefix: &str) {
    assert!(
...
"#,
    ...
    assert_context_output_bounded(output, "Variable truncated: False. Variable: ");
...
"#,
    ...
    let items = custom_tool_output_items(&second_mock.single_request(), "call-1");
    let output = text_item(&items, /*index*/ 1);
    assert_context_output_bounded(output, "Variable truncated: True. Variable: ");
