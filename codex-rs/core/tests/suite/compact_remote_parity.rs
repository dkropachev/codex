...
fn normalize_tmp_prefix_before_marker(text: &mut String, marker: &str) {
    ...
    while let Some(relative_marker_index) = text[search_start..].find(marker) {
        ...
        let start = prefix
            ...
            .or_else(|| {
                prefix.rfind("/.tmp").and_then(|temp_index| {
                    let path_start = prefix[..temp_index]
                        .rfind(|ch: char| ch.is_whitespace() || matches!(ch, '"' | '\'' | '('))
                        .map_or(0, |index| index + 1);
                    (path_start < temp_index).then_some(path_start)
                })
            })
            .or_else(|| {
                prefix.rfind("\\.tmp").and_then(|temp_index| {
...
#[test]
fn normalize_string_rewrites_custom_tmpdir_skill_paths() {
    let text = normalize_string(
        "file: /extra/dkropachev/tmp-codex-tests/.tmp5YYdK3/skills/.system/imagegen/SKILL.md and \
         C:\\agent\\tmp\\.tmpiP36Yr\\skills\\custom\\SKILL.md",
    );
