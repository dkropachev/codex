...
Workflows should provide user-facing UX while they run. Call `ctx.progress(message, data?)` for bounded progress updates. Standalone CLI execution renders progress on stderr; hosted execution keeps progress separate from the final result. Return structured output from `run`, then let the required `format(output, { format: "markdown.v1" })` method produce the single final user-visible markdown result. Hosted workflows may also call the optional `ctx.requestUserInput(...)` hook.

Hosted workflow execution uses app-server v2 `thread/workflowCommand` and the normal turn lifecycle. The server emits `turn/started`, records the formatted result through `item/started` and `item/completed`, and finishes with `turn/completed`; interruption uses the existing turn interruption API. Do not invent separate `workflowRun/*` methods or treat the JavaScript runner's private control channel as an app-server API.
...
The canonical workflow roots are `$CODEX_HOME/workflows` and `.codex/workflows`. Each workflow directory is a self-contained git repository containing `workflow.yaml`, `package.json`, `.gitignore`, `README.md`, `DESIGN.md`, `src/workflow.ts`, `src/tests/`, and `state/.gitkeep`. `.gitignore` must ignore runtime state while allowing `state/.gitkeep`; runtime state files must not be tracked. `workflow.yaml` uses API version 1 and keeps metadata plus structured `validation.commands` entries (`program` and `args`) and coverage markers. Validation rejects legacy runtime fields such as `command`, `userDescription`, `api`, and `usage.options`.

Workflow API contracts are TypeScript-first. `src/workflow.ts` exports `WorkflowInput`, `WorkflowOutput`, `inputSchema`, and `outputSchema`, and its default export is a `defineWorkflow({...})` object with `apiVersion: 1`, `id`, `title`, `callableName`, `run(ctx, input)`, optional `complete(ctx, request)`, and `format(output, { format: "markdown.v1" })`. Both schemas explicitly declare Draft 2020-12. CLI, TUI aliases, and app-server runs all normalize top-level input into the same JSON object: `--input <JSON>` or bounded `--input @file` supplies a base object, repeated kebab-case flags form arrays, explicit flags override base keys, and `workingDirectory` is injected only when absent. Because that field is additive, the v1 top-level object contract must not use `maxProperties`. Positional `{ argv, text }` parsing is not supported.

Completion hooks receive `{ input, activeField?, prefix, mode: "field" | "value" }` and return `{ value, description? }[]`. The TUI derives static fields and enum/const values from the top-level input schema, then merges and deduplicates the optional hook results for only the active workflow under output and time limits. Every workflow keeps `workflow.yaml` aligned with its `validation.coverage` contract; the required set includes `positive`, `load`, `autocomplete`, `negative`, and `recovery` when recovery behavior exists. `/workflow validate <id>` checks metadata agreement, required files and layout, gitignore policy, schemas and exports, isolated import/load behavior, undeclared or non-local dependencies, coverage markers, and every configured validation command. A successful validation prints exactly `valid`. Workflows must not rely on globally installed or non-local third-party packages. Use workflow paths only after a workflow is identified; do not run broad file search, web search, or unrelated repo spelunking to rediscover existing workflows or the workflow system.
...
- `WF-007`: `src/workflow.ts` must own the workflow API contract; its schemas, completion hook, and `markdown.v1` formatter must stay explicit and accurate.
...
After accepted edits, validate the workflow with the most specific available workflow validation path. For implementation-stage changes, that means running `/workflow validate <id>` or `codex workflow validate <id>` before considering the workflow ready; validation must deterministically check metadata agreement, package layout and docs, ignored runtime state, dependency locality, Draft 2020-12 schemas, required exports and loadability, coverage markers, and the workflow's configured validation commands. Treat validation output other than exactly `valid` as a repair blocker. If validation fails, repair within the configured `[workflows]` repair policy; deterministic fixes should run before AI fallback, and every repair cycle must rerun validation before the workflow is considered ready. When config is absent, use at most three repair cycles. Stop and report the remaining failure when the repair budget is exhausted or the fix requires a user decision.
