...
Workflow implementation must happen inside a discoverable workflow directory only. For new workflows, run `/workflow develop <description>` first so Codex creates the package under the configured workflow root, then pass that workflow directory to the architect and coder. Never ask workflow agents to create `workflow.yaml`, `src/`, `package.json`, or other workflow package files in the caller workspace root. Workflow architect, coder, and reviewer turns are write-confined to the detected workflow directory; if no ancestor contains `workflow.yaml`, they cannot write and must ask the parent to scaffold or provide the correct workflow path.
