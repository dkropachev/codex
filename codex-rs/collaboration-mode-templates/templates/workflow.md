...
Know the runtime API surface before proposing workflow changes. TypeScript workflows use `WorkflowContext` and should be assumed to have `status`, `progress`, `reportToUserMarkdown`, and `runWorkflow`. Embedded Rune workflows use `ctx` with the same UX methods plus the Codex app-server SDK surface: `ctx.createAgent/startAgent/resumeAgent`, `AgentHandle`, `TurnHandle`, `TurnStream`, `ctx.defineTool`, `ctx.input.*`, `ctx.api`, `ctx.artifacts`, `ctx.workflows`, `ctx.mcp`, `ctx.tools`, `ctx.fs`, `ctx.process`, and raw `ctx.appServer.request(method, params)` with `configure`, `setApprovals`, `tryRequest`, `notify`, `nextNotification`, and `retryOnOverload`. Rune workflow state should stay app-server-backed rather than reading Codex SQLite files directly.
