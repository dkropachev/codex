...
The canonical workflow roots are `$CODEX_HOME/workflows`, `.codex/workflows`, and `[workflows].search_paths`. Each workflow directory is its own git repo with `workflow.yaml`, `README.md`, `DESIGN.md`, and workflow source files. Workflow layout rules are strict: source code lives under `src/`, tests live under `src/tests/`, and persistent state or database files live under `state/`. `.gitignore` must ignore `artifacts/` and `state/*` while allowing `state/.gitkeep`; runtime state files must not be tracked. Workflow API contracts are TS-first: `src/workflow.ts` must export `WorkflowInput`, `WorkflowOutput`, and default `defineWorkflow<Input, Output>({ id?, title?, callableName?, run, complete?, format? })`. Put field docs, enum values, input/output schema shape, completion hooks, formatter contracts, and custom workflow errors in TypeScript, not in YAML. Keep `workflow.yaml` for metadata, dependencies, validation, repair, tool registration, and git policy; validation rejects legacy `workflow.yaml.api` schemas and `usage.options`. CLI, TUI aliases, and app-server runs all normalize top-level TypeScript input fields into the same JSON object, with kebab-case flags, `--input JSON` or `--input @file` as the base object, and explicit flags overriding base keys. Completion hooks receive structured `complete(ctx, request)` data with partial input, active field, prefix, and `field`/`value` mode; do not author raw `{ argv, text }` parsing in workflow code. Every object output schema must declare non-empty `properties` or an explicit `additionalProperties` policy, and workflows with `validation.contractSmoke` must emit stdout JSON that matches the published contract. Every workflow must keep `workflow.yaml` aligned with its `validation.coverage` contract, and each test file should declare the markers it covers with `// workflow-covers: ...`. The required coverage set includes `positive`, `load`, `autocomplete`, `negative`, and `recovery` when recovery exists. `/workflow validate <id>` should be understood to check docs, layout, runtime state ignore/tracking, coverage markers, local packages, loadability, autocomplete readiness, output schema shape, configured contract smoke, and the workflow's own validation commands/tests. Workflows must not rely on globally installed packages. Built-in platform modules are allowed, but third-party packages must be declared in the workflow's local `package.json` and resolved from that workflow directory's own `node_modules`. Use those paths only after a workflow is identified; do not run broad file search, web search, or unrelated repo spelunking to rediscover existing workflows or the workflow system.
...
- `WF-007`: `src/workflow.ts` must own the workflow API contract; validation commands, output schemas, and contract smoke validation must stay explicit and accurate.
