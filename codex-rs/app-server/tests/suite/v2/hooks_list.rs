...
async fn plugin_upgrade_refreshes_hook_runtime_for_loaded_session() -> Result<()> {
  ...
  "plugins": [{
    ...
    assert_eq!(
        ...
        Some(format!("python3 {}", expected_hook_path.display()).as_str())
    );
    assert!(!hook.r#async);
    assert!(!hook.enabled);
...
"#,
    ...
    let expected_hook_path = new_plugin_root
        ...
        .canonicalize()?;
    assert_eq!(data[0].hooks[0].handler_type, HookHandlerType::Command);
    assert_eq!(
        data[0].hooks[0].command.as_deref(),
        Some(format!("python3 {}", expected_hook_path.display()).as_str())
    );
    assert!(!data[0].hooks[0].r#async);
    ...
    assert_eq!(
        ...
        vec![HooksListEntry {
            ...
            hooks: vec![HookMetadata {
                ...
                command: None,
                r#async: false,
                server: Some("security".to_string()),
...
async fn hooks_list_uses_root_repo_hooks_for_linked_worktrees() -> Result<()> {
    ...
    assert_eq!(repo_hook.handler_type, HookHandlerType::Command);
    assert_eq!(repo_hook.command.as_deref(), Some("echo root hook"));
    assert!(!repo_hook.r#async);
    assert_eq!(worktree_hook.handler_type, HookHandlerType::Command);
    assert_eq!(worktree_hook.command.as_deref(), Some("echo root hook"));
    assert!(!worktree_hook.r#async);
