...
fn build_test_tracing() -> (
    ...
    (Arc::new(TestTracing { exporter, provider }), subscriber)
}
...
struct TracingHarness {
    ...
    tracing: Arc<TestTracing>,
}

impl TracingHarness {
    async fn new(tracing: Arc<TestTracing>) -> Result<Self> {
        Self::new_with_origin(ConnectionOrigin::WebSocket, tracing).await
    }

    async fn new_with_origin(origin: ConnectionOrigin, tracing: Arc<TestTracing>) -> Result<Self> {
...
fn run_current_thread_test_with_stack<F, Fut>(name: &str, test: F) -> Result<()>
where
    F: FnOnce(Arc<TestTracing>) -> Fut + Send + 'static,
    Fut: Future<Output = Result<()>> + Send + 'static,
{
    ...
    let handle = std::thread::Builder::new()
        ...
        .spawn(move || -> Result<()> {
            // Keep tracing state per test and give these support assertions enough stack for
            // debug builds, where deeply nested JSON-RPC fixtures otherwise overflow.
            global::set_text_map_propagator(TraceContextPropagator::new());
            let (tracing, subscriber) = build_test_tracing();
            let runtime = tokio::runtime::Builder::new_current_thread()
                .enable_all()
                .build()?;
            tracing::subscriber::with_default(subscriber, || {
                runtime.block_on(Box::pin(test(tracing)))
            })
        })?;
...
fn thread_start_jsonrpc_span_exports_server_span_and_parents_children() -> Result<()> {
    run_current_thread_test_with_stack(
        ...
        |tracing| async move {
            let mut harness = TracingHarness::new(tracing).await?;
...
fn remote_control_origin_rejects_device_key_requests() -> Result<()> {
    run_current_thread_test_with_stack(
        "remote_control_origin_rejects_device_key_requests",
        |tracing| async move {
            let mut harness =
                TracingHarness::new_with_origin(ConnectionOrigin::RemoteControl, tracing).await?;
