...
MCP tool hooks appear with `handlerType: "mcpTool"`. Their `server` and `tool` fields identify the configured MCP target, while `command` is `null`. Command hooks populate `command` and report `server` and `tool` as `null`. These nullable fields remain present for wire compatibility.
