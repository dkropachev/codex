...
- `thread/workflowCommand` — run a local workflow package with a JSON object input. Codex requests the workflow's `tui.markdown.v1` formatter, emits the result as a normal assistant message, and records that message in thread history for subsequent turns. Returns `{}` immediately while standard turn/item notifications stream the formatted output.
