...
The JavaScript workflow runtime uses Bun as its TypeScript execution engine. It treats `src/workflow.ts` as the canonical contract: export `WorkflowInput` and `WorkflowOutput`, then default-export `defineWorkflow<Input, Output>({ id?, title?, callableName?, run, complete?, format? })`. Validation extracts the TypeScript input/output schemas, completion hook metadata, and formatter metadata from that source; legacy `workflow.yaml.api` and `usage.options` metadata are rejected. The optional `complete(ctx, request)` hook receives structured partial input, the active field, the typed value prefix, and a `field` or `value` mode instead of raw argv text. CLI, TUI aliases, and app-server runs normalize to the same JSON input object; top-level schema fields become kebab-case flags, `--input JSON` or `--input @file` supplies a base object, and explicit flags override those keys. `WorkflowContext.status(status)` reports structured live workflow status, `WorkflowContext.progress(message, data?)` remains a legacy compatibility helper that maps into workflow status text, and `WorkflowContext.reportToUserMarkdown(markdown)` remains a legacy markdown handoff. A structured status update carries `workflowName`, `workflowStatus`, and an optional list of thread rows (`{ name, status }`). The TUI renders a single-thread workflow as `Workflow <workflowName>: <workflowStatus>` and only expands to thread rows when more than one thread status is present. Compiled-in Rust workflows receive the same normalized input and status/markdown event stream plus a native state directory, cancellation token, and model candidate catalog. The built-in `dev-cycle` workflow stores experiment state in `$CODEX_HOME/workflows/.native-workflows/dev-cycle/state/dev_cycle.sqlite3`; when an agent-capable host is available it orchestrates planner, writer, reviewer, verifier, test, and integrator stages, and otherwise returns a structured blocked result instead of a preview. The runner uses local `node_modules/.bin/bun` when a workflow supplies one; otherwise it uses Codex's pinned managed Bun at `CODEX_HOME/workflows/.bin/bun`. Codex-launched Bun processes get isolated Bun install, cache, global package, and runtime transpiler cache paths under `CODEX_HOME/workflows/.env`. When the managed Bun binary is missing, Codex downloads the pinned Bun package into `CODEX_HOME/workflows/.bin/`; validation command execution prepends that directory to `PATH`, so scaffolded `bun build` and `bun test` commands do not require a user-installed Bun binary. The TUI renders the published formatter first, then legacy markdown handoffs, and falls back to pretty JSON when no formatter is present.
...
- `thread/codexConfigIntent/submit` - submit a `/codex <request>` style AI-backed config intent for a loaded thread. Clients may pass `context` with source-of-truth command/module details such as generated slash-command registry output. Codex supplements that with the embedded Codex guide, current `codex --help` output, and a writable scratch workspace while treating the target workspace as read-only. The visible user message remains only the user-authored `intent`; generated Codex context is delivered as developer instructions. Codex config intents may inspect and run scripts, writing only to the scratch workspace, `/tmp`, or the Codex config directory. They may emit one `<proposed_plan>` when planning is appropriate; explicit apply intents run in the config-edit submode with the same filesystem limits. When deterministic wording rules do not classify the intent, Codex uses an AI classification fallback that chooses investigation vs config-edit under those limits. Returns `{}` when the turn is accepted for execution.
...
- `turn/start` — add user input to a thread and begin Codex generation; responds with the initial `turn` object and streams `turn/started`, `item/*`, and `turn/completed` notifications. Prefer experimental `permissions` profile selection for permission overrides; the legacy `sandboxPolicy` field is still accepted but cannot be combined with `permissions`. Experimental `promptContext` can override developer/user-context prompt blocks for this and subsequent turns; `systemInstructions` is only accepted on `thread/start`, `thread/resume`, and `thread/promptContext/update`, and `strict` defaults to `true`. Experimental `toolPolicy` can narrow tool visibility and reject matching invocations for this and subsequent turns. For `collaborationMode`, `settings.developer_instructions: null` means "use built-in instructions for the selected mode".
...
- `command/exec` — run a single command under the server sandbox without starting a thread/turn (handy for utilities and validation). Optional `toolPolicy` can override invocation policy for this standalone command.
...
- `experimentalFeature/enablement/set` — patch the in-memory process-wide runtime feature enablement for the currently supported feature keys (`apps`, `memories`, `plugins`, `remote_control`, `tool_router`, `tool_suggest`, `tool_call_mcp_elicitation`). For each feature, precedence is: cloud requirements > --enable <feature_name> > config.toml > experimentalFeature/enablement/set (new) > code default.
...
- `workflow/list`, `workflow/read`, `workflow/impact` — discover git-backed TypeScript workflows from `$CODEX_HOME/workflows`, `.codex/workflows`, and `[workflows].search_paths`; read `workflow.yaml`/`README.md`; and report dependency plus git impact. Workflow summaries surface `command` when `workflow.yaml.command` is set, or a fallback alias for simple ids without `/`. They also expose cached `commandOptionHints` derived from the last validated TypeScript contract when available. Pass optional `stageSessionId` to overlay one staged authoring session without mutating the live workflow tree.
- `workflow/develop`, `workflow/edit`, `workflow/validate`, `workflow/repair` — scaffold, update, validate, or repair a workflow using the same command engine as `codex workflow` and `/workflow`. `workflow/develop` accepts optional `id`, `command`, `title`, and `location` (`project` or `global`) fields so clients can create a deterministic workflow path, alias, and root instead of deriving them from prose or `[workflows].default_location`. Command-style workflow responses include `exitCode`; invalid validation responses use `exitCode: 1` while still returning the validation message. Validation checks workflow layout (`src/`, `src/tests/`, `state/`), runtime state ignore/tracking rules, explicit object output schema shape, unfinished scaffold markers, raw develop flags in metadata/docs, `latest` dependency versions, configured `validation.contractSmoke` output, and workflow validation commands/tests; live validation then extracts and publishes the TS contract and regenerates the local generated workflow client module before reporting readiness. `workflow/repair` runs the validation-guided repair loop and keeps iterating until validation is clean or the configured repair budget is exhausted. It emits `workflowRun/progress` notifications while resolving, validating, applying deterministic fixes, refreshing local npm dependencies when package or missing-module repairs need it, running AI fallback, committing, and completing. Its final response returns typed `workflow`, `validation`, `validationCommandResults`, and `repair` fields, where `validationCommandResults` carries the raw stdout/stderr for each validation command run, `repair` includes the stop reason, applied fixes, remaining findings, blocked findings, unsupported findings, and repair cycle counters, and the human-readable message summarizes the applied fixes or stop reason with failure context. Pass optional `stageSessionId` to keep edits in a private staging area until the client publishes or discards them.
- `workflowRun/start`, `workflowRun/read`, `workflowRun/wait`, `workflowRun/cancel` — start an app-server-owned asynchronous workflow run, read its in-memory state, wait for completion or timeout, or cancel it. `workflowRun/start` returns immediately with `{ run }`; progress, markdown handoff, completion, and failure are streamed as `workflowRun/progress`, `workflowRun/reportToUserMarkdown`, `workflowRun/completed`, and `workflowRun/failed` notifications on the same app-server connection. Native Rust runs also receive cancellation and model-candidate context; `dev-cycle` uses those candidates for review/verifier model selection and persists run evidence in its native SQLite state DB.
- `workflow/publish`, `workflow/discard` — publish or discard a staged workflow authoring session identified by `stageSessionId`. `publish` merges staged changes back into the live workflow roots after validation. `discard` removes the staged session roots without touching the live workflows.
- `workflow/config/read`, `workflow/config/write` — inspect or edit `[workflows]` config values.
- `workflow/command/execute`, `workflow/authoringContext/prepare` — execute the shared workflow command parser or prepare registry/config context for workflow authoring clients. The parser recognizes the same workflow aliases used by `/<cmd>` and `codex <cmd>`, supports `develop --location project|global`, and rejects accidentally quoted develop flag blobs. Pass optional `stageSessionId` to keep a multi-call authoring session on one staged workflow view. `workflow/command/execute` still accepts `args: ["publish"]`, `args: ["discard"]`, or `args: ["done"]`, but dedicated `workflow/publish` and `workflow/discard` are the clearer API. `workflow/command/execute` responses include `exitCode` so clients can render non-zero command results as failures while preserving the command message.
...
{ "method": "thread/start", "id": 10, "params": {
    ...
    "toolPolicy": {
        ...
        "invocation": {
            "mode": "default",
            "rules": [{
                "id": "no-root-agents-scan",
                "effect": "deny",
                "tools": ["exec_command"],
                "when": {
                    "jsonPath": "$.cmd",
                    "regex": "\\bfind\\b.*(/|/tmp).*AGENTS\\.md"
                },
                "message": "Search AGENTS.md only inside the repo/worktree."
            }]
        }
    },
...
- `toolPolicy.invocation` is evaluated before tool handlers run. `mode: "default"` applies built-in default deny rules plus supplied rules; `mode: "unrestricted"` skips default and supplied invocation rules; `mode: "allowOnly"` denies selected invocations unless an `effect: "allow"` rule matches. `mode: "deny"` is equivalent to default deny-rule evaluation.
- Invocation rules select by `tools` and/or `mcp`. A rule without `when` matches every selected invocation. In `when`, omitted `jsonPath` defaults to `""`, which matches the whole raw tool input. Non-empty `jsonPath` supports `$` and `$.field[.field...]`; `regex` is validated by the server before the policy is accepted.
...
- `account/rateLimits/read` — fetch ChatGPT rate limits; updates arrive via `account/rateLimits/updated` (notify). When an account pool is active, this also refreshes pool member usage.
...
{ "id": 1, "result": { "account": { "type": "chatgptPool", "id": "codex-pro", "activeAccountId": "work-pro", "members": [{ "id": "work-pro", "email": "work@example.com", "planType": "pro", "active": true, "unavailableReason": null, "regularRemaining": 42, "sparkRemaining": 100, "lastError": null }] }, "requiresOpenaiAuth": true } }
```
...
- `chatgptPool` is returned when `[account_pool]` is active. `id` is the pool id, `activeAccountId` is the currently selected member when known, and `members` describes each configured account.
- Pool member fields are `id`, `email`, `planType`, `active`, `unavailableReason`, `regularRemaining`, `sparkRemaining`, and `lastError`.
...
- With an active ChatGPT account pool, `account/rateLimits/read` refreshes pool member usage before returning the current account rate limits.
