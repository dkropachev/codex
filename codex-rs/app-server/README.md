...
For runtime discovery, use `apiCatalog/read` over app-server or `codex api` from the CLI. The catalog is JSON intended for IDEs, coding agents, and workflow code; it includes app-server methods, configured MCP servers and tool schemas, built-in workflow helpers, the workflow runtime surface, and discovered workflow metadata, including each workflow's command alias and runtime when one is registered.

Workflows declare `workflow.yaml.runtime.kind` as `rune` or `typescript`; omitted legacy manifests are treated as TypeScript with `src/workflow.ts`. New `workflow/develop` scaffolds Rune by default unless `runtime: "typescript"` is supplied. Runtime entrypoints must be relative paths inside the workflow directory. Rune workflows run in-process through the embedded Rust runtime, use `src/workflow.rn` by default, call `pub async fn run(ctx, input)`, may add `complete(ctx, input)` and `to_tui_markdown(result)`, and publish contracts from `workflow.yaml` API schemas. TypeScript workflows keep the resident Node host, named default-export async entrypoint, optional `complete(...)`, and optional `WorkflowOutput.toTuiMarkdown(result)`. Both runtimes emit the same structured status events and markdown reports; the TUI renders published formatters first, then legacy markdown handoffs, and falls back to pretty JSON when no formatter is present.
...
- `apiCatalog/read` — return a machine-readable catalog of app-server methods, configured MCP servers/tools, built-in workflow helpers, workflow SDK/runtime symbols, and discovered workflows. Optional `include` selects sections and `mcpDetail: "toolsAndAuthOnly"` skips MCP resource inventory.
- `workflow/list`, `workflow/read`, `workflow/impact` — discover git-backed workflows from `$CODEX_HOME/workflows`, `.codex/workflows`, and `[workflows].search_paths`; read `workflow.yaml`/`README.md`; and report dependency plus git impact. Workflow summaries surface `runtime`, `command` when `workflow.yaml.command` is set, or a fallback alias for simple ids without `/`. They also expose cached `commandOptionHints` derived from the last validated contract when available, with legacy `usage.options` or `api.inputSchema` metadata used as a migration fallback. Pass optional `stageSessionId` to overlay one staged authoring session without mutating the live workflow tree.
- `workflow/develop`, `workflow/edit`, `workflow/run`, `workflow/validate`, `workflow/repair` — scaffold, update, execute, validate, or repair a workflow using the same command engine as `codex workflow` and `/workflow`. `workflow/develop` accepts optional `runtime` and defaults to Rune. Validation checks runtime-aware required files and entrypoint containment, workflow layout (`src/`, `src/tests/`, `state/`), TypeScript local package imports when applicable, Rune source compilation, runtime state ignore/tracking rules, JSON schema syntax, explicit object output schema shape, `validation.contractSmoke`, and workflow validation commands/tests. Rune validation requires `validation.contractSmoke`, supports multiple smoke cases with `input`, `expectOutput`, and `expectError`, validates declared format schemas by calling `to_tui_markdown(result)`, and checks `complete(ctx, input)` when autocomplete coverage is enabled. Rune validation commands default to the built-in `codex workflow test-rune` runner, which compiles the workflow and exported test functions under `src/tests/**/*.test.rn` and `src/tests/**/*.spec.rn`; legacy `true` commands are treated as that built-in runner. Live validation publishes the TypeScript-extracted or Rune manifest-defined contract only when validation passes and the source digest is current, then regenerates the local generated workflow client module before reporting readiness. `workflow/repair` runs the validation-guided repair loop and keeps iterating until validation is clean or the configured repair budget is exhausted. It emits `workflow/progress` notifications while resolving, validating, applying deterministic fixes, refreshing local npm dependencies when package or missing-module repairs need it, running AI fallback, committing, and completing. Its final response returns typed `workflow`, `validation`, `validationCommandResults`, and `repair` fields, where `validationCommandResults` carries the raw stdout/stderr for each validation command run, `repair` includes the stop reason, applied fixes, remaining findings, blocked findings, unsupported findings, and repair cycle counters, and the human-readable message summarizes the applied fixes or stop reason with failure context. Pass optional `stageSessionId` to keep edits in a private staging area until the client publishes or discards them.
- `workflow/repair` can also fall back to an AI repair pass scoped to the workflow directory when deterministic fixes cannot handle unsupported findings; that fallback can appear in `appliedFixes` as `aiRepair`.
...
- `workflow/command/execute`, `workflow/authoringContext/prepare` — execute the shared workflow command parser or prepare registry/config context for workflow authoring clients. The parser recognizes the same workflow aliases used by `/<cmd>` and `codex <cmd>`. Pass optional `stageSessionId` to keep a multi-call authoring session on one staged workflow view. `workflow/command/execute` still accepts `args: ["publish"]`, `args: ["discard"]`, or `args: ["done"]`, but dedicated `workflow/publish` and `workflow/discard` are the clearer API.
