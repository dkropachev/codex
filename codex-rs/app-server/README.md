...
- `thread/codexConfigIntent/submit` - submit a `/codex <request>` style AI-backed config intent for a loaded thread. Clients may pass `context` with source-of-truth command/module details such as generated slash-command registry output. Codex supplements that with the embedded Codex guide, current `codex --help` output, and a writable scratch workspace while treating the target workspace as read-only. The visible user message remains only the user-authored `intent`; generated Codex context is delivered as developer instructions. Investigation intents answer read-only. Explicit config-edit intents run in a plan-like submode that must emit one `<proposed_plan>` and must not write config until a later apply flow. When deterministic wording rules do not classify the intent, Codex uses an AI classification fallback that first chooses investigation vs config-edit under the same read-only sandbox. Returns `{}` when the turn is accepted for execution.
