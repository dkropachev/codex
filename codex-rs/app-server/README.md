...
Workflows declare `workflow.yaml.runtime.kind` as `rune` or `typescript`; omitted legacy manifests are treated as TypeScript with `src/workflow.ts`. New `workflow/develop` scaffolds Rune by default unless `runtime: "typescript"` is supplied. Rune workflows run in-process through the embedded Rust runtime, use `src/workflow.rn` by default, call `pub async fn run(ctx, input)`, may add `complete(ctx, input)` and `to_tui_markdown(result)`, and publish contracts from `workflow.yaml` API schemas. TypeScript workflows keep the resident Node host, named default-export async entrypoint, optional `complete(...)`, and optional `WorkflowOutput.toTuiMarkdown(result)`. Both runtimes emit the same structured status events and markdown reports; the TUI renders published formatters first, then legacy markdown handoffs, and falls back to pretty JSON when no formatter is present.
