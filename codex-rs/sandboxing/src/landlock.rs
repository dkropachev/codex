...
/// Converts the split sandbox policies into the CLI invocation for
/// `codex-linux-sandbox`.
///
/// The helper still accepts the legacy `SandboxPolicy` projection while newer
/// callers also pass the canonical filesystem and network policy pair.
#[allow(clippy::too_many_arguments)]
