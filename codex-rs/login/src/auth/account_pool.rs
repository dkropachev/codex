...
impl AccountPool {
    async fn select_auth(&self, bucket: AccountPoolBucket) -> Option<CodexAuth> {
        for member in self.select_members(bucket) {
            let Some(auth) = member_auth(member).await else {
        ...
        for member in &self.members {
            ...
            match fetch_usage_remaining(base_url, &auth).await {
                Ok((regular_remaining, spark_remaining)) => {
                    ...
                    self.clear_last_error(&member.account_id);
                }
                Err(err) => self.set_last_error(&member.account_id, err),
            }
        }
    }
...
async fn member_auth(member: &AccountPoolMember) -> Option<CodexAuth> {
    let auth = member.manager.auth_cached_unpooled()?;
    if let Err(err) = member.manager.refresh_token().await {
        tracing::debug!(
            account_id = member.account_id,
            "failed to refresh account pool member token: {err}"
        );
    }
    let auth = member.manager.auth_cached_unpooled().unwrap_or(auth);
    if let Err(err) = auth.initialize_runtime(None).await {
        tracing::debug!(
            account_id = member.account_id,
            "failed to initialize account pool member auth runtime: {err}"
        );
        return None;
    }
    Some(auth)
