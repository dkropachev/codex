...
pub(crate) fn discover_handlers(
    ...
    _codex_self_exe: Option<&Path>,
    _codex_home: Option<&Path>,
    _workflows_config: Option<&WorkflowsConfigToml>,
) -> DiscoveryResult {
    ...
    if let Some(config_layer_stack) = config_layer_stack {
        ...
        // Workflow quality is enforced by workflow authoring commands before
        // staged changes are published or committed, not by a PostToolUse hook.
