...
- `data` - ordered list of collaboration mode masks (partial settings to apply on top of the base mode)
  ...
  - Built-in presets do not set `model`. The Plan preset sets `reasoning_effort` to medium; the Workflow preset also uses medium reasoning and is returned only when `[features].workflows = true`. Clients keep or override model separately.
