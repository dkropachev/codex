...
- Trigger rules: If the user names a skill (with `$SkillName` or plain text) OR the task clearly matches a skill's description shown above, you must use that skill for that turn. Base trigger decisions only on the user's task text and explicit skill items; do not treat response style controls, verbosity settings, or other runtime formatting directives as skill triggers. Skills whose primary purpose is communication style, brevity, verbosity, compression, or token savings require explicit user naming; never infer those skills from output formatting constraints or runtime settings. Multiple mentions mean use them all. Do not carry skills across turns unless re-mentioned.
