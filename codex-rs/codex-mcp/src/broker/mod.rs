...
const SOCKET_VERSION_DIR: &str = "v2";
...
pub(crate) fn reusable_stdio_identity(
    ...
    if config.process_reuse_scope == McpServerProcessReuseScope::None {
        return Ok(None);
    }
    ...
    validate_process_reuse_scope(config.process_reuse_scope, cwd.as_deref())?;
    ...
    let reuse_partition_key = reuse_partition_key(
        config.process_reuse_scope,
        &launch.cwd,
        runtime_environment.project_root_markers(),
    );
...
fn validate_process_reuse_scope(
    scope: McpServerProcessReuseScope,
    configured_cwd: Option<&Path>,
) -> Result<()> {
    if scope != McpServerProcessReuseScope::User {
        return Ok(());
    }
    let Some(configured_cwd) = configured_cwd else {
        return Err(anyhow!(
            "process_reuse_scope = \"user\" requires an explicit stdio cwd"
        ));
    };
    if !configured_cwd.is_absolute() {
        return Err(anyhow!(
            "process_reuse_scope = \"user\" requires an absolute stdio cwd"
        ));
    }
    Ok(())
}

fn reuse_partition_key(
    scope: McpServerProcessReuseScope,
    launch_cwd: &str,
    project_root_markers: &[String],
) -> String {
    let cwd = Path::new(launch_cwd);
    match scope {
        McpServerProcessReuseScope::None => "none".to_string(),
        McpServerProcessReuseScope::Cwd => cwd_partition_key(launch_cwd),
        McpServerProcessReuseScope::Project => find_project_root(cwd, project_root_markers)
            .map(|root| path_partition_key("project", &root))
            .unwrap_or_else(|| cwd_partition_key(launch_cwd)),
        McpServerProcessReuseScope::Repo => get_git_repo_root(cwd)
            .map(|root| path_partition_key("repo", &root))
            .or_else(|| {
                find_project_root(cwd, project_root_markers)
                    .map(|root| path_partition_key("project", &root))
            })
            .unwrap_or_else(|| cwd_partition_key(launch_cwd)),
        McpServerProcessReuseScope::User => "user".to_string(),
    }
}

fn find_project_root(cwd: &Path, project_root_markers: &[String]) -> Option<PathBuf> {
...
fn cwd_partition_key(launch_cwd: &str) -> String {
    format!("cwd:{launch_cwd}")
}

fn path_partition_key(scope: &str, path: &Path) -> String {
