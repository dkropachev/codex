...
pub struct ModelsResponse {
    #[serde(serialize_with = "serialize_model_infos_with_legacy_base")]
...
impl<'de> Deserialize<'de> for ModelsResponse {
        ...
        D: Deserializer<'de>,
    {
        ...
        if value.get("models").is_some() {
            return serde_json::from_value::<CodexModelsResponse>(value)
                .map(|response| Self {
        ...
        if value.get("data").is_some() {
            return serde_json::from_value::<OpenAiModelsListResponse>(value)
                .map(|response| Self {
                    models: response
                        .data
                        .into_iter()
        ...
        Err(D::Error::custom(
            "expected a Codex models response with `models` or an OpenAI models list with `data`",
        ))
    }
}
