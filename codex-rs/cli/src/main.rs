        ...
        Some(Subcommand::Login(mut login_cli)) => {
            ...
            match login_cli.action {
                ...
                None => {
                    ...
                    {
                        eprintln!(
                            "Choose one login credential source: --with-api-key, --with-access-token, or --with-agent-identity."
                        );
                        std::process::exit(1);
                        ...
                        if login_cli.account.is_some() {
                            eprintln!("--account cannot be used with --with-agent-identity");
                            std::process::exit(1);
                        }
                        ...
                        if login_cli.account.is_some() {
                            eprintln!("--account cannot be used with --with-access-token");
                            std::process::exit(1);
                        }
...
mod tests {
    ...
    #[test]
    fn logout_rejects_account_with_all() {
    ...
    #[test]
    fn account_refresh_accepts_pool_selector() {
        let cli =
            MultitoolCli::try_parse_from(["codex", "account", "refresh", "--pool", "codex-pro"])
                .expect("parse should succeed");

        let Some(Subcommand::Account(account)) = cli.subcommand else {
            panic!("expected account subcommand");
        };
        let AccountSubcommand::Refresh(refresh) = account.subcommand else {
            panic!("expected account refresh subcommand");
        };
        assert_eq!(refresh.id, None);
        assert_eq!(refresh.pool.as_deref(), Some("codex-pro"));
    }
    ...
    fn root_strict_config_is_rejected_for_unsupported_subcommands() {
        ...
        .expect_err("account should not support root --strict-config");

        assert_eq!(
            err.to_string(),
            "`--strict-config` is not supported for `codex account`"
        );
    }
