...
impl StateRuntime {
    pub async fn tool_router_tune_observations(
        ...
        let since_created_at_ms = match window {
            ToolRouterDiagnosticsWindow::Lifetime => i64::MIN,
...
mod tests {
    ...
    async fn tune_observations_include_route_tool_outcome_and_shape_breakdowns() {
        ...
        fallback.request_shape_json = Some(serde_json::to_string(&shape).expect("shape json"));
        runtime
        ...
        error.request_shape_json = Some(serde_json::to_string(&shape).expect("shape json"));
        runtime
        ...
        let observations = runtime
            .tool_router_tune_observations(ToolRouterDiagnosticsWindow::Lifetime, Some("gpt-test"))
            .await
            .expect("observations");
