...
#[tokio::test]
async fn released_fork_migration_history_upgrades_without_rewriting_versions() {
    ...
    let released_history = sqlx::query(
        "SELECT version, description, checksum FROM _sqlx_migrations WHERE version IN (41, 42) ORDER BY version",
    )
    .fetch_all(&pool)
    .await
    .expect("released fork migration history should load")
    .into_iter()
    .map(|row| {
        (
    ...
    assert_eq!(
        released_history
            .iter()
            .map(|(version, description, _)| (*version, description.as_str()))
            .collect::<Vec<_>>(),
        vec![
            (41, "tool router output optimizations"),
            (42, "tool router ledger compaction filter"),
        ]
    );
    ...
    .fetch_all(&pool)
    .await
    .expect("upgraded fork migration history should load")
    .into_iter()
    .map(|row| {
        (
            row.get::<i64, _>("version"),
    ...
    let added_migrations = sqlx::query(
        "SELECT version, description FROM _sqlx_migrations WHERE version >= 48 ORDER BY version",
    )
    .fetch_all(&pool)
    .await
    .expect("new migration history should load")
    .into_iter()
    .map(|row| {
        (
            row.get::<i64, _>("version"),
...
SELECT
    EXISTS(
        ...
        WHERE type = 'table' AND name = 'tool_router_output_optimizations'
    ) AS has_output_optimizations,
    EXISTS(
        SELECT 1 FROM pragma_table_info('tool_router_ledger')
        WHERE name = 'output_compaction_filter'
    ) AS has_compaction_filter,
    EXISTS(
    ...
    assert_eq!(
        (
            schema.get::<i64, _>("has_output_optimizations"),
