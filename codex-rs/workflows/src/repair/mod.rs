...
fn build_fix_plan(
    ...
    for finding in &report.findings {
        match finding {
            ...
            WorkflowValidationFinding::LegacyWorkflowApiMetadata { .. } => {
                plan.update_validation_yaml = true;
            }
...
fn apply_validation_yaml_fix(
    ...
    if plan.update_validation_yaml {
        ...
        if !spec.api.is_null() {
            spec.api = JsonValue::Null;
        }
        let clear_usage = if let JsonValue::Object(usage) = &mut spec.usage {
            usage.remove("options").is_some() && usage.is_empty()
        } else {
            false
        };
        if clear_usage {
            spec.usage = JsonValue::Null;
        }
        changed = true;
    }
